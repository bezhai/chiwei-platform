...
import { LaneBindingResolver, type LaneBindingStore } from './resolver';

// LaneBindingResolver 是渠道无关的泳道决策能力。它的入参只认统一概念
// （channel key + common conversation + 全局 bot 标识），绝不接触任何渠道的原始
// 字段。读取靠注入的结构型 LaneBindingStore（ORM-free，运行时由 TypeORM 实现
// 注入），所以本测试用内存 fake 顶替真实 DB，纯跑、不连库。
//
// 这个 fake 忠实模拟真实 lane_routing 表的查询语义：按「common conversation」
// 优先查，再按「全局 bot 标识」查；没绑定返回 null。解析器据此决定 lane：
// 命中返回 lane_name，未命中返回 'prod'。

// 一个可计数、可重设绑定的内存 store，记录被查了几次，用来验证缓存命中不打 DB。
class FakeLaneBindingStore implements LaneBindingStore {
    findChatLaneCalls = 0;
    findBotLaneCalls = 0;
...
describe('LaneBindingResolver.resolveLane — 渠道无关的 chat/bot 泳道决策', () => {
    it('chat 命中绑定：优先返回 chat lane，不再查 bot', async () => {
        const store = new FakeLaneBindingStore();
        store.setChatBinding('018f-chat', 'ppe-chat');
        store.setBotBinding('赤尾', 'ppe-bot');
        const router = new LaneBindingResolver(store);

        const lane = await router.resolveLane('channel-x', '赤尾', '018f-chat');
    ...
    it('bot 命中绑定：返回绑定的 lane_name', async () => {
        ...
        const lane = await router.resolveLane('channel-x', '赤尾', undefined);
    ...
    it('bot 未命中绑定：默认回 prod（不返回 null、不抛错）', async () => {
        ...
        const lane = await router.resolveLane('channel-x', '没绑定的bot', undefined);
    ...
    it('30s TTL 内重复决策走缓存：同一 bot 不重复查 DB', async () => {
        ...
        store.setBotBinding('赤尾', 'ppe-foo');
        let clock = NOW;
        const router = new LaneBindingResolver(store, () => clock);

        expect(await router.resolveLane('channel-x', '赤尾', undefined)).toBe('ppe-foo');
        expect(await router.resolveLane('channel-x', '赤尾', undefined)).toBe('ppe-foo');
    ...
    it('未命中（prod 默认）同样进缓存，避免每条消息为未绑定 bot 反复打 DB', async () => {
        ...
        expect(await router.resolveLane('channel-x', '没绑定', undefined)).toBe('prod');
        expect(await router.resolveLane('channel-x', '没绑定', undefined)).toBe('prod');
        expect(store.findBotLaneCalls).toBe(1);
    });
    ...
    it('不同 channel 的同名 bot 是不同缓存条目（缓存 key 含 channel，不串）', async () => {
        ...
        // 的同名 bot 应触发一次新的 DB 查询，而不是命中前一个 channel 的缓存。
        await router.resolveLane('channel-y', '赤尾', undefined);
        expect(store.findBotLaneCalls).toBe(2);
    });
    ...
    it('clearCache 后重新查 DB（admin 改绑定后主动失效，不必等 30s）', async () => {
        ...
        expect(await router.resolveLane('channel-x', '赤尾', undefined)).toBe('ppe-bar');
        expect(store.findBotLaneCalls).toBe(2);
    });
    ...
    it('clearCache 后解绑也立即生效：原绑定移除后回到 prod 默认', async () => {
        ...
        expect(await router.resolveLane('channel-x', '赤尾', undefined)).toBe('prod');
    });
});

describe('渠道无关红线 — 决策能力源码不含任何渠道原始字段名', () => {
    it('resolver.ts 不出现 chat_id/open_id/event/sender 等渠道消息体字段名', () => {
        const src = readFileSync(join(import.meta.dir, 'resolver.ts'), 'utf8');
        // 这些是 IM 渠道原始消息体里的典型字段名；决策只认 channel key + common
        // 口径标识，源码里一旦出现它们，就说明决策又退回去碰渠道原始字段了。
        ...
        for (const f of FORBIDDEN_RAW_FIELDS) {
