...
import type { ChatRequestPayload } from './chat-request';
...
// 阶段不实际 publish —— 只把"该发什么"登记下来，由渠道侧接线点在
// 该渠道入站消息写入成功之后再发 MQ（保证下游按 message_id 查内容时先存后查、
...
// makeTextReply 构造为闭包（仓储逻辑仍只在 chat-request.ts 一处），
// 但**不在 runRules 阶段执行**。渠道侧接线点抢到去重锁后才调用它，
...
// 只有聊天主链路 makeTextReply 是真正渠道无关、默认对所有渠道生效的核心指令。
// 它经 CommandRegistry.registerCore 注册，由 forChannel 拼接在「该 channel 自己的
// 指令之后」，作为 NeedRobotMention 的 catch-all 兜底。
// 渠道专属的 utility 指令归各自渠道插件，靠 register(channel, ...) 决定归属 ——
// 引擎不认识任何具体渠道的指令。
