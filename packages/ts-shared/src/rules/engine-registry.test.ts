...
//   3. 另一个 channel 不会命中前一个 channel 注册的渠道指令。
...
describe('runRulesWith 经 CommandRegistry.forChannel 分发', () => {
    it('channel-x 消息：命中 channel-x 注册的渠道指令', async () => {
        const fired: string[] = [];
        const reg = new CommandRegistry();
        reg.register('channel-x', [recordingCmd('x-balance', true, fired)]);
        reg.registerCore([recordingCmd('chat', true, fired)]);

        const state = await runRulesWith(msg({ channel: 'channel-x' }), {
            chatRules: reg.forChannel('channel-x'),
            botRole: undefined,
            notBlocked: async () => true,
        });

        expect(state.kind).toBe('responded');
        expect(state.matchedRule).toBe('x-balance');
        expect(fired).toEqual(['x-balance']);
    });
    ...
    it('核心聊天主链路始终在最后、作为 catch-all 兜底', async () => {
        ...
        reg.register('channel-x', [recordingCmd('x-balance', false, fired)]);
        reg.registerCore([recordingCmd('chat', true, fired)]);
    ...
    it('channel-y 消息：不会命中 channel-x 注册的渠道指令（归属隔离）', async () => {
        const fired: string[] = [];
        const reg = new CommandRegistry();
        reg.register('channel-x', [recordingCmd('x-balance', true, fired)]);
        reg.registerCore([recordingCmd('chat', true, fired)]);

        const state = await runRulesWith(msg({ channel: 'channel-y' }), {
            chatRules: reg.forChannel('channel-y'),
            botRole: undefined,
            notBlocked: async () => true,
        });

        // channel-y 的 forChannel 只含核心通用指令 → 命中聊天主链路，绝不命中 x-balance。
        expect(state.matchedRule).toBe('chat');
        expect(fired).toEqual(['chat']);
        expect(fired).not.toContain('x-balance');
    });
});
