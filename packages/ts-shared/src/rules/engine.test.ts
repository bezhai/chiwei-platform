...
describe('runRules single terminal-state exit', () => {
    it('NotBlocked=false -> exactly one terminal state: blocked', async () => {
        ...
        expect(st.channel).toBe('channel-x');
    });
...
describe('runRules channel filtering + 渠道声明', () => {
    it('另一个 channel 的消息不命中限定 channel 的规则，收敛 to no_match (recorded skip)', async () => {
        let handled = false;
        const rules: RuleConfig[] = [
            {
                rules: [alwaysPass],
        ...
        const st = await runRulesWith(msg({ channel: 'channel-y' }), {
            chatRules: rules,
            botRole: undefined,
            notBlocked: async () => true,
        });
    ...
    it('限定 channel 的规则仍然命中该 channel 的消息', async () => {
        const rules: RuleConfig[] = [
            {
                rules: [alwaysPass],
                handler: async () => {},
        ...
        const st = await runRulesWith(msg({ channel: 'channel-x' }), {
            chatRules: rules,
            botRole: undefined,
            notBlocked: async () => true,
        });
