...
**决策四：把"消息是否触发 bot"的判断契约平台化，从原来的非目标提升为核心；并要求 runRules 收敛成单一终态出口模型，禁止任何无终态记录的静默退出。**
...
经一轮 codex T1 评审深挖后，这条决策真正的闭环机制需要写清楚：仅靠"AddressingPolicy 在前面做总闸判定 + 无规则匹配时记一条日志"是不够的，因为 `runRules` 内部还存在多个会继续静默丢弃消息的退出点——黑名单挡掉、渠道过滤跳过、异步规则不通过、bot 角色 / category 不匹配、handler 抛异常被吞、fallthrough 直接 break 等等，每一个都可能让一条消息既没产生响应、也没留下任何可查记录就消失。所以决策四不止是"加一层前置判定"，它要求 `runRules` 改成**单一终态出口模型**：每一条进入 `runRules` 的消息，无论最终走哪条退出路径，都必须收敛到一个唯一、明确、可查的终态记录——它要么记录"响应了什么"，要么记录"为什么没响应"，二者必居其一，禁止存在任何不写终态记录就 `break` / `return` 的静默退出点。

合起来看，决策四由两部分共同构成、缺一不可：一是 `AddressingPolicy` 作为前置总闸，对新渠道能真正拦截并带上拒绝原因；二是 `runRules` 的单一终态出口，把飞书侧原本散落的所有静默退出点全部消灭。只有这两者同时成立，"禁止静默丢弃"才真正落地，而不是停留在口号上。

**决策五：channel 抽象切成四层契约，且消息进来后归一化成平台无关的统一形态，channel-server 内的规则引擎在统一形态上平台无关运行。**

四层契约这部分仍然成立：入站事件 adapter、平台无关的内部消息模型、出站回复 adapter、bot 命中契约，代码里的飞书耦合恰好集中在这四个地方，平台差异通过 adapter 把原始入站归一化成统一的 `InboundMessage`（带 channel、全局内部 ID、统一内容）后收敛在 adapter 内部，agent-service 这条 AI 中间链路对消息来自哪个平台确实无感知，不用动。

但有一处原方案翻转了，需要说清楚演进。原本设想 channel-server 里那套规则引擎（`runRules`）保持飞书 native 不动、新的 channel 抽象只在旁边旁挂，让"中间链路一律不动"。实践中发现这条不成立：规则引擎还吃飞书专属富对象的话，新的"渠道命中判断"对飞书就只是个并行的纯观测摆设、并没真正接管判断；而 QQ 这类新渠道的消息根本进不了飞书专属的 `runRules`，它们"这条要不要回"的响应判定就彻底没有着落。所以用户推翻了"中间 runRules 不动"这个设计，改定如下：

- `runRules` 不再是飞书专属引擎，而是**一套平台无关的统一规则引擎，所有渠道走同一套**。消息一进来就由 adapter 归一化成上面那个平台无关的统一形态，到统一消息层时已经平台无关，`runRules` 消费的是这个统一形态，不再吃飞书专属富对象。
- 关于"每条指令支持哪些渠道"，原本的设想是"每条指令声明支持渠道、不声明就默认全平台"。经 codex 实查代码确认，这个默认值定反了：原本以为可以跨渠道复用的"帮助 / 指令 / 复读"等指令，实际上都深度绑死飞书——它们引用飞书模板、飞书 SDK、飞书卡片、`union_id`、admin 判定、群配置等飞书专属东西，根本不是平台无关的。如果让它们默认全平台，QQ 消息进来会去触发这些飞书专属逻辑，要么报错要么行为错乱。所以首版范围收紧：**只有 persona 文本主链路（用户找赤尾聊天这条路）是真正平台无关的，做平台无关化、默认全平台；其余所有指令，凡引用了飞书 SDK / 卡片 / 实体或读飞书专属字段的（帮助、指令、复读、余额、撤回、水群、发图、表情包等 utility / 卡片类指令），首版一律显式声明仅支持 `lark`，内部逻辑不重写、行为保持零变化**。
- 判断准则一句话：碰到飞书 SDK / card / 实体 / 专属字段的指令就标记成 lark-only、内部一行不动；只有真正平台无关的 persona 文本主链路才默认全平台。首版以保守为先，宁可把一条指令标成 lark-only，也不冒飞书行为回归的风险。
- `runRules` 分发时按当前消息的 channel 过滤：消息渠道不在某条指令声明的支持渠道内，就跳过（miss）这条指令。比如 QQ 消息会自然 miss 掉那些只声明支持 `lark` 的飞书专属指令。
- **不为非飞书渠道写防御性降级**。万一某个飞书专属能力在不支持它的渠道上被执行到了，就让它自然 fail（QQ 发卡片 failed 就 failed），不加兜底、不写优雅降级逻辑——渠道过滤已经把绝大多数情况挡在前面，剩下的边角让它如实失败比藏一层降级更可查。
...
channel 入站（webhook / 长连接 / 同步 HTTP 请求，由 adapter 内部封装）
  ...
  → channel-server（这条链路的顺序是钉死的，不能调换）：
      1. InboundAdapter[channel].verify(raw)          验签
      2. InboundAdapter[channel].parse(raw)           原始入站 → 通用 InboundMessage
                                                       （到这里消息已归一化为平台无关统一形态）
      3. AddressingPolicy[channel].decide/enforce(...) 前置总闸：这条消息要不要 bot 响应
                                                       不响应：记可查日志后丢弃（禁止静默）
      4. IdentityResolver.resolve(InboundMessage)      channel 内 ID → 全局内部 ID
      5. runRules(InboundMessage)                      平台无关的统一规则引擎，单一终态出口；
                                                       消费统一形态，每条指令声明支持渠道，
                                                       按当前消息 channel 过滤跳过不支持的指令
                                                       （不写防御降级）；无论走哪条退出路径都
                                                       收敛到唯一可查终态记录
      6. 存 conversation_messages（全部用全局 ID）
      7. 发 ChatTrigger（带 channel + 全局 ID）到 RabbitMQ
  → agent-service：对消息来自哪个 channel 完全无感知，AI 链路逻辑不动
...
这条链路的顺序是经 codex T1 评审后定稿、钉死不能调换的，理由是后面几步彼此有强依赖：`AddressingPolicy` 的 `decide/enforce` 必须前置在 `IdentityResolver.resolve` 之前，因为它是不依赖 DB 的纯判定，即使后面身份解析所依赖的 DB 出异常，这一步也已经先产出了"这条消息为什么不回"的可查结论；而 `IdentityResolver.resolve` 又必须前置在 `runRules` / 存库 / 发 MQ 之前，因为黑名单等规则要靠全局 ID 才查得到、存库和发 MQ 必须带全局 ID 而不能发裸 channel ID，resolve 不先做，这些步骤要么查不到要么会把裸 ID 漏下去，fail-loud 也就无从成立。简言之：先判定要不要回（不依赖 DB，DB 挂了也能解释原因），再换全局 ID（后续一切都依赖它），再跑规则引擎、存库、发 MQ。

`channel-proxy` 和 `channel-server` 这两个进程本身完全 channel 无关。所有 channel 差异都收敛在 `InboundAdapter` / `OutboundAdapter` / `AddressingPolicy` 这三个按 channel 分实现的东西里，加上一个 channel 无关的 `IdentityResolver` 负责 ID 翻译。这里要强调一点（对应决策五的翻转）：channel-server 里的规则引擎 `runRules` 不靠"飞书 native 不动、adapter 旁挂"来做到平台无关，而是因为消息在第 2 步已经被 adapter 归一化成了平台无关的统一形态，`runRules` 直接在这个统一形态上运行，对所有渠道都是同一套引擎；它通过"每条指令声明支持渠道、按当前消息 channel 过滤跳过"来表达渠道差异，而不是把飞书专属逻辑藏在引擎里。
...
- channel-server `infrastructure/integrations/memory.ts` 的 `storeMessage()`：入站消息真正落库的地方，往 conversation_messages 写 user_id（存的是飞书 union_id）、chat_id 等飞书裸 ID，T5 必须改成写全局内部 ID。
...
建用户、会话、消息三套映射表，停机原地重写那 7 张表加 Qdrant，飞书侧的 `(lark, 原飞书ID)` 进映射表，把"调用方覆盖"里所有需要改的调用方都改完，不留 fallback。产出是三套映射表、带 drain gate 的迁移过程、DB 和 Qdrant 的快照与恢复预案、调用方改造。接线边界：把新 channel 抽象接入真实链路时，要让消息在入口就被 adapter 归一化成平台无关的统一形态再往下走，channel-server 里的 `runRules` 随之改成在统一形态上平台无关运行——每条指令声明自己支持哪些渠道（不声明默认全平台）、`runRules` 按当前消息 channel 过滤跳过不支持的指令，不为非飞书渠道写防御降级（落到不支持渠道的飞书专属能力让它自然 fail）。同时飞书非文字消息既有的全部路径必须保持存活、零回归——非文字消息照常存库、图片照常进识图管线、赤尾照常能看图聊天，绝不能因为接入新抽象或 runRules 平台无关化就把非文字消息的处理路径丢掉。

接线中段（这里记为 5b，即新 channel 抽象代码侧已接进真实链路、但 DB 身份映射表尚未 apply 的那个阶段）是一个**不可部署的中间态**：此时契约链路里 IdentityResolver 没有真实映射表可查，契约链必然失败，所以 5b 不能作为一个可独立部署、可独立 coe 验证的状态——真正的端到端真机验证统一并入 coe（记为 5e，即映射表已 apply、drain gate 走完之后的状态）一次性做。5b 阶段的验证口径因此放在代码侧：用 fake / in-memory 的 IdentityResolver 跑契约链路的契约测试、跑 `runRules` 单一终态出口的测试（每条退出路径都必须留下唯一可查终态记录）、跑飞书逐场景的行为回归用例。另外补一条贯穿整条契约链的 fail-loud 铁律：渠道契约链（parse / decide / resolve）中任意一步失败时，该条消息必须 fail-loud——不写库、不发 MQ、记一条可查的错误日志，绝不允许退回飞书裸 ID 继续往下走，因为那会把"渠道化失败"伪装成"一条正常飞书消息"，污染数据且彻底丧失可查性。这里要把 fail-loud 的边界讲清楚，否则它会跟"飞书行为零变化"打架：fail-loud 约束的范围仅限"契约相关的写入与下发"，也就是不写带全局 ID 的库、不发带全局 ID 的 MQ、不拿飞书裸 ID 冒充全局 ID 往下游传。它**不约束**飞书 native 那些既有的、跟新身份契约毫无关系、本来就用飞书裸 chat_id / message_id 的副作用——识图管线请求、bot_chat_presence upsert、MessageTransferer 这一类。这些副作用在现状里就排在契约链之前执行，按"飞书行为零变化"这条命门，它们必须原样、原位置（仍然在契约链之前）继续跑，契约链后面失不失败都不影响它们。特别要强调的是：**绝不能为了迁就 fail-loud 把这些飞书 native 副作用挪到契约链成功之后**。因为 5b 本身就是 DB 映射表还没 apply 的不可部署中间态，这个阶段契约链必然失败，一旦把副作用挪到契约成功之后，5b 阶段飞书识图、在线状态这些副作用就会全部不执行，等于飞书行为整体回归，直接违反命门。

验收标准：coe 泳道迁移后，飞书用户和会话的历史查询结果跟迁移前一致；新写入用的是全局 ID；Qdrant 按新 ID 检索正确；快照恢复演练成功；代码里没有双读或 fallback 路径。另外，因为 T5 完成了把新 channel 抽象接进真实链路，这里必须承接从 T2 并过来的那部分验收：在 coe 泳道绑飞书 dev bot 做一次飞书行为零变化的端到端回归，覆盖私聊、群聊、@bot、不 @bot 各个场景跟改造前完全一致，并且非文字消息（图片、富文本、sticker、语音、文件）的存库、图片进识图管线、赤尾看图聊天这三条路径全部零回归，要有实际收发的日志或截图为证。
