...
- **A0. 统一节点执行契约**（B 阶段共同前提）：单点定义 `@node` 函数签名（输入/返回值）、自动 emit 语义、异常如何上浮、`on_error` 声明的语义、wire 链上各装饰器（durable/transient/debounce/fan_out_per）的组合规则、错误分类边界（runtime / wire / node / tool / capability 各自负责哪一档）。**产物是一份 contract 文档 + 一份契约违反的运行时断言**。没有这个 contract，B1/B4/B7/B8 会互相覆盖语义。
...
- **B4. @node(on_error=...)**（依赖 A0 + A3）：节点级声明错误处理（swallow_and_log / dlq / raise），语义由 A0 contract 定义。验收：所有现有节点的 try-except 全部移除，演练剧本"某节点强制 raise，按声明路由到 DLQ 而非吞掉"通过。
...
- **B8. @node 兑现自动 emit**（依赖 A0）：装饰器真正包装函数，返回 Data 自动 emit 到图里（commit 0ea263b 已临时修过，这里按 A0 contract 重新整理）。验收：单元测试覆盖"返回 Data 自动流入下游节点"+ 演练剧本"chat → safety → recall 链路 emit 自动衔接"通过。
