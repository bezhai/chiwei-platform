...
flowchart TB
    ...
    fetch["fetch agent · 每日抓取"] --> materials
    ...
    chapter -- "长弧走到哪" --> world
    materials -- "今天的真实世界" --> world
    worldstate -- "上一版 detail" --> world

    world -- "推演新一版" --> worldstate
    world -- "翻页了就更新：考完了 / 放榜了 / 搬家了" --> chapter
    ...
    class chapter,sediment,review newpart
```
...
**第一块：世界长弧。** 新增「世界的长弧走到哪」持久化版本链（自然语言、不按人归属，WorldState 的慢层），world 推演到翻页事件时更新，world 的推演输入接上这层。验收口径：不手动改 persona 的前提下，world 自己把「高考结束」写进长弧，次日她不再被推去上学——眼前这个穿帮本身就是第一个真实验收用例。

**第二块：记忆沉淀**（已有规划，这里只说衔接）。它就是缺失的天级钟：每天的意识流收拢成一页可回看的经历，往上滚成周、月。既是 review 的输入前提——否则 review 面对几千条原始意识流没法回头看——也让 world 推演终于有「昨天」可读，而不是只剩上一版 detail 里碰巧带着的残影。

**第三块：周月级 persona review。** 依赖前两块。读这段日子的沉淀经历 + 当前 persona，产出新版本 persona，diff 留痕可见。
