...
**十一、`/block` 拉黑不生效——写的和读的不是同一个 id 体系。**

共享的 `NotBlocked` 规则按 `message.commonUserId` 去查 `user_blacklist.union_id`，它的注释自己写明了"列名是历史遗留，值口径已收敛为 common_user_id"。而 `/block` 指令存进去的是被 @ 那个人在飞书 mention 里的 id。两个 id 体系不同，于是这一行永远匹配不上：管理员看到"拉黑成功"，被拉黑的人继续正常对话。

D4 迁移时按纪律照搬了（行为一致是这批唯一的验收口径），并在代码里就地记了这件事。修它要先决定拉黑的权威 id 是哪一个、以及历史数据怎么处理，是独立议题。
