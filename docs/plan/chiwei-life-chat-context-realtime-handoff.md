...
> 给新会话接手用。当前在分支 `feat/chiwei-group-proactive`,改动**未 commit**。
> 本文记录 chat→life 对话感知重构的落地状态。
...
event**;群聊改成单独 `emit(EventArrived)` 纯唤醒(私聊不唤醒、她自然醒来时实时拉到)。
这样未读重置、回复落库时序两个坑都不存在(读取那一刻库里是什么就看到什么)。
...
- `app/data/message_record.py`(+29):新增两个 dataclass `LifeChatMessage`
  (`speaker_display_name`/`is_self`/`text`/`cst_time`)、`LifeChatConversation`
...
- `app/nodes/life_wake.py`(+71):新增 `_format_recent_chats(conversations)` 渲染函数 +
  ...
  (notebook 段之后、时间锚之前,`try` 调查询 + **fail-soft**,`since_ms = int(now.timestamp()*1000) - 6h`)。
  - 渲染:按会话分块(群标群名、群名缺失兜底「· 一个群里：」**不拼 None**;私聊「· 一段私聊里：」),
    每条「（时间）发言人：内容」,`is_self` → 「我」。**忠实呈现、不加工成叙述、不截断单条**。
  - fail-soft(查询失败 → logger.warning + 段缺席、本轮照常跑)**已被 closed-loop test 13 passed
...
- 已新增 `_wake_life_for_group_chat`:群(非 p2p)过白名单 `should_feed_chat_to_life` 后
  `emit(EventArrived(lane=current_deployment_lane() or "prod", persona_id=...))`;私聊不唤醒;
...
- 旧测试 `tests/nodes/test_chat_node_conversation_replay.py` 和
  ...
  `tests/nodes/test_chat_node_group_wake.py` 覆盖群白名单内 emit、白名单外不 emit、私聊不 emit、
...
**closed-loop test 范式**(`tests/integration/test_world_life_closed_loop.py`,现 13 个用例):
...
## 当前真实落盘状态(git status,已多次双查确认)

```
 M apps/agent-service/app/data/message_record.py
 M apps/agent-service/app/data/queries/mailbox.py
 M apps/agent-service/app/data/queries/messages.py
 M apps/agent-service/app/domain/world_events.py
 M apps/agent-service/app/infra/cst_time.py
 M apps/agent-service/app/life/feed_whitelist.py
 M apps/agent-service/app/nodes/chat_node.py
 M apps/agent-service/app/nodes/life_wake.py
 M apps/agent-service/tests/domain/test_event_delivery.py
 M apps/agent-service/tests/domain/test_event_mailbox.py
 M apps/agent-service/tests/integration/test_world_life_closed_loop.py
 M apps/agent-service/tests/life/test_feed_whitelist.py
 D apps/agent-service/tests/nodes/test_chat_node_conversation_replay.py
 D apps/agent-service/tests/nodes/test_chat_node_life_feed_whitelist.py
 M apps/agent-service/tests/nodes/test_life_wake.py
?? apps/agent-service/tests/data/test_persona_related_chats.py
?? apps/agent-service/tests/nodes/test_chat_node_group_wake.py
?? apps/agent-service/tests/nodes/test_life_wake_recent_chats.py
```

分支 `feat/chiwei-group-proactive`,未 commit。当前已跑:
- `uv run pytest tests/nodes/test_chat_node_group_wake.py tests/life/test_feed_whitelist.py tests/domain/test_event_delivery.py tests/domain/test_event_mailbox.py tests/nodes/test_life_wake.py -q` → 142 passed
- `uv run pytest tests/nodes/test_chat_node_group_wake.py tests/nodes/test_life_wake_recent_chats.py tests/data/test_persona_related_chats.py tests/integration/test_world_life_closed_loop.py -q` → 33 passed
