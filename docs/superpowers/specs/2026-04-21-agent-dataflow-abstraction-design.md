...
Node 里**允许**出现：
- 调用 capability：`LLMClient`, `AgentRunner`, `EmbedderClient`, `VectorStore`, 各类实体 Store（`LifeStateStore` / `MessageStore` / ...）, `HTTPClient`……
...
业务代码只看到「能力」，看不到「实现」。**Store 按业务实体特化**——通用 `Store[T]` 会退化成 `store.get("life_state:xxx")` 的字符串 key 拼接，等于换壳不换芯，不要。
...
| `VectorStore` | `store.search(vec, k) / store.upsert(frag)` | qdrant / pgvector |
| `LifeStateStore` | `get(persona_id) / upsert(persona_id, state)` | pg |
| `MessageStore` | `get(message_id) / append(msg) / recent_by_chat(...)` | pg |
| `ScheduleStore` | `get(persona_id, date) / upsert(...)` | pg |
| `NoteStore` | `find_by_chat(chat_id) / upsert(...)` | pg |
| `HTTPClient` | `http.get(url, ...)` | httpx，带 lane routing / trace |

新增 Data 通常意味着新增一个对应的 `XxxStore` capability（如果这个 Data 需要持久化）。按实体拆分的代价是 capability 数量多，但**签名清晰、类型强、无字符串耦合**，值得。
...
1. **Phase 0 — Runtime 骨架 + Capability 层 + Legacy Bridge**：
   - Data / Node / Wire / Stream[T] 基类；runtime 最小实现；capability adapters（`LLMClient`, `AgentRunner`, `EmbedderClient`, `VectorStore`, `LifeStateStore`, `MessageStore`, ...）
...
- **采纳并写进本 spec**：流式一等公民、Data identity/key、多 producer、Placement 绑定 PaaS App、按域拆 wiring、行为不变量、流终结语义、rebuild 重入 dedup、trace 穿越 durable、Phase 顺序 + Legacy Bridge、as_latest 一致性机制、Store 按实体特化、受控并发合法、行为不变量分类、non-goal 措辞、部署行为变更声明
