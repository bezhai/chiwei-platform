...
1. 改 `infra/test-env/docker-compose.yaml`（新增 qdrant + mongo service，rabbitmq image 改 fixed digest，commit）
2. cpu1 拉起两个新容器（`docker-compose up -d`）+ 健康检查
3. 通过 paas-engine API **扩展现有** `qdrant` + `mongo` bundle（保留 prod baseline keys + 新加 `class_overrides[coe]` + `required_keys[coe]`）。**App `config_bundles` 引用不动**（5 App 早已引用 mongo/qdrant）。
4. 烟雾测试：
   - prod lane resolved-config byte-equal（5 App × 改动前后 diff 全空）
   - coe-validation lane resolved-config 真派 chiwei-test endpoint
5. 端到端：部 agent-service + vectorize-worker + lark-server + chat-response-worker + recall-worker 到 coe-validation lane
6. 验证：
   - agent-service 启动 log 显示真连 chiwei-test qdrant（10.37.6.235:16333），不是 prod qdrant
   - chiwei-test-qdrant 真有 4 个 chiwei collection（before/after diff 或 collection 详细信息含 vectors_count > 0）
   - chiwei-test-mongo `db.lark_event` 真有 dev bot 发的 event（最新一条 _id ObjectId 时间戳 ≥ deploy T0）
   - prod qdrant 零变化（按 collection 详细 endpoint 查 vectors_count + 看 agent-service log 中 qdrant URL）
   - prod mongo 零变化（dev bot app_id 在 T0 后 prod mongo 中 0 条 event）
7. PR 发到 main，无 prod 行为变化

**注**：RequiredKeys[coe] 反向 reject 机制 Phase 2 已验证（PR #218 ppe-phase-2-validation lane HTTP 400 反向 reject 测试通过），Phase 3 不再重复测——避免在 prod PaaS DB 上临时写坏 bundle 的窗口风险。
