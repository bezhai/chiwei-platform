...
**状态**: Draft v2 (2026-04-28，已吸收 reviewer 第 2 轮意见)
...
async def run_post_safety(req: PostSafetyRequest) -> Recall | None:
    """audit + 写 safety_status + 决定是否撤回，单节点完成。

    业务幂等：节点开头查 safety_status；已 "passed" / "blocked" 直接 short-circuit。
    DLQ replay 时（adoption mode 跳过 runtime dedup），节点会被重跑，
    短路逻辑保证不会重复审计已审完的消息。

    顺序：先 emit Recall (return) → wire 自动 sink mq → 再写 status。
    若 sink 失败，handler 抛 → DLQ → status 仍为 "pending" → replay 时重跑
    audit 并重新 emit Recall（dup recall 已知，运维 replay runbook 处理）。
    """
    current = await get_safety_status(req.session_id)
    if current in ("passed", "blocked"):
        logger.info(
    ...
    if decision.is_blocked:
        # Recall 先返回让 wire 触发 sink；写 status 在 sink 之后由调用方串
        # —— 但 @node 装饰器是 emit-then-return，没有"sink 之后"的钩子。
        # 解决：在 sink 失败抛异常的语义下，set_safety_status 必须在
        # emit Recall **之后**才能执行，否则 status="blocked" 会让 replay
        # 短路掉重跑 → 永久丢 recall。所以这里不通过 return 走 sink，
        # 改成手动 emit Recall + 写 status 串行。
        recall = Recall(
        ...
        await emit(recall)  # 失败抛 → DLQ → replay → 短路查 status 仍 pending → 重跑
        await set_safety_status(req.session_id, "blocked",
                                {"checked_at": checked_at, "reason": decision.reason})
        return None  # 已显式 emit，不让 @node 装饰器重复 emit
    else:
...
**节点设计要点**：
- 业务幂等用 `safety_status` 短路；不依赖 runtime dedup（adoption mode 已跳过 insert_idempotent）
- emit Recall **早于** set_safety_status：让"recall 已发但 status 写失败"的情况只是 replay 重发（dup），而不是"status 写了但 recall 没发"的永久丢失
- DLQ replay 注意事项写进 runbook（见 4.4）
...
Adoption mode 下 runtime 不做 dedup，consumer 每次都跑。`run_post_safety` 节点入口靠 `safety_status` 短路：

| Status (replay 入口) | replay 时节点行为 |
|---|---|
| pending（默认） | 跑 audit + emit Recall (if blocked) + 写 status |
| running | （目前没用到，预留状态机扩展位）当 pending 处理 |
| passed | 短路 return None |
| blocked | 短路 return None — Recall 已发过 |

**已知边界 case**：
- 节点中途崩溃（emit Recall 之前抛）→ status 仍 pending → replay 时重跑 audit + 重新 emit Recall — 正确
- emit Recall 成功，set_safety_status 之前抛 → status 仍 pending → replay 时短路逻辑判 pending → **重跑 audit + 重新 emit Recall（dup！）**
  - lark-server recall-worker 第二次执行：消息已撤回，飞书 API 返回 already-recalled，`recalledCount=0`，被标记 `recall_failed`（覆盖第一次的 `recalled` 状态，是 UX 偏差不是数据丢失）
  - 接受偏差的理由：DLQ replay 是**人工操作**（runbook `docs/superpowers/runbooks/2026-04-dlq-replay.md`），不是自动重试
  - 缓解：runbook 加注释 — replay PostSafetyRequest 之前先 `SELECT safety_status FROM agent_responses WHERE session_id=?`，已 blocked 的不要 replay

**lark-server 自身 recall 幂等**是更彻底的修法（基于 trigger_message_id 去重），但**不在 Phase 2 范围**，作为 followup 跟踪。
...
- `run_post_safety`：
  - mock `get_safety_status` 返回 "passed" / "blocked" 验证短路路径
  - mock `_run_audit` + `set_safety_status` + `emit`，验证 blocked 顺序（emit Recall → write status）
  - mock 顺序：emit 抛异常 → status 不写
...
- [ ] `agent_responses.safety_status` 在新链路下从 `pending` 正确更新到 `passed` / `blocked`
