...
**状态**: Draft v6 (2026-04-28，已吸收 reviewer 第 6 轮意见)
...
async def run_pre_safety_via_graph(
    ...
    2. 用 asyncio.wait({fut, emit_task}, FIRST_COMPLETED) 而不是
       ...
       - fut 先完成时直接拿 verdict
    3. emit_task 收尾用 ``completed`` 标记区分三条退出路径（reviewer round 6）：
       - 拿到 verdict 后才置 completed=True；finally 不 cancel emit_task
         (此时它通常已 done，await 等收尾 + 收 exception)
       - timeout / emit 早失败 / **外层 cancel** → completed 仍为 False，
         finally 一律 cancel emit_task + suppress CancelledError
       - 没有 completed 标记的版本会让"外层取消"路径漏掉 emit_task，
         导致 LLM 检查白跑 + waiter 被 cleanup 后 verdict resolve 变 no-op
    """
    ...
    try:
        ...
        if not done:
            ...
            # 正常路径：等 emit_task 收尾（通常已 done），传播任何 exception
        ...
        # timeout / emit 早失败 / 外层 cancel 都走这里 cancel emit_task
    ...
    # fail-open（timeout 或 emit 早失败；外层 cancel 时 finally 后 CancelledError
    # 会自动 propagate，不会走到这里）
...
**并发重复 PostSafetyRequest 边界**（reviewer round 6 issue 2）：

Adoption mode 下 runtime 不做 dedup。如果两条 `PostSafetyRequest` (同一 session_id) 几乎同时进入 `run_post_safety`，两个 consumer 协程都可能在 `get_safety_status` 阶段看到 status="pending"，然后**都跑 audit + 都 emit Recall**，产生 dup audit (多一次 LLM 调用) + dup recall。

Phase 2 接受这个边界，**不加 checking/running 状态机做 CAS 抢占**：
- 上游正常路径只 emit 一次：`schedule_post_actions` (apps/agent-service/app/chat/post_actions.py:55) 在每次 chat 完成时只调一次 `_publish_post_check`。runtime 的 in-process emit 是同步 dispatch，没有 retry 重发
- 唯一可能并发的场景：DLQ 同时 replay 同一条消息两次（人工误操作）— runbook 已规定 "落 terminal 状态不要 replay"，pending 状态下 replay 仅在节点真的卡住时才发生
- 加状态机的代价（schema 加字段、CAS UPDATE、consumer 边界处理）超过当前真实风险

**强幂等**作为 followup 跟踪：跟 lark-server recall 幂等 (trigger_message_id 去重) 一起做更合理 — 两层去重比一层 checking 状态更可靠。
...
- `run_pre_safety_via_graph`：
  - 节点正常完成：返回 verdict（completed=True，emit_task 不被 cancel）
  - 节点超时（mock 节点 sleep > 21s）：fail-open + emit_task 被 cancel
  - emit 自身抛异常：fail-open + 异常被 log
  - **外层调用方 cancel**（例如 chat pipeline 整体取消 pre_task）：emit_task 被 cancel + waiter 被 cleanup（reviewer round 6 issue 1）
