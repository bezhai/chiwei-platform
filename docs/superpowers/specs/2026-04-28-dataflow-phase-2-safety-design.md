...
**状态**: Draft v4 (2026-04-28，已吸收 reviewer 第 4 轮意见)
...
async def run_pre_safety_via_graph(
    ...
    超时实现要点：emit() 是 in-process dispatch，会同步 await 完整链路
    (run_pre_safety -> @node 自动 emit verdict -> resolve_pre_safety_waiter
    -> set Future result)。如果节点卡住，``await emit(...)`` 也卡住，
    wait_for 永远等不到机会触发。所以必须把 emit 包进独立 task，让超时
    检查跑在调用方协程上。shield(fut) 防止 wait_for 超时时把 future 标
    成 cancelled — emit_task 后续可能仍能 set_result，cancelled 状态会
    让 set_result 抛 InvalidStateError。
    """
    ...
    try:
        return await asyncio.wait_for(asyncio.shield(fut), timeout=21.0)
    except TimeoutError:
        logger.warning("pre safety timeout: pre_request_id=%s", pre_request_id)
    except Exception as e:
        logger.warning(
            "pre safety failed: pre_request_id=%s, error=%s", pre_request_id, e
        )
    finally:
        if not emit_task.done():
            ...
            logger.error(
                "pre safety emit failed: pre_request_id=%s, error=%s",
