...
实现方式：在 ConfigBundle 上加一个 `RequiredClasses []string` 字段，标记哪些 lane class 必须有 override：
...
type ConfigBundle struct {
    ...
    LaneOverrides   map[string]map[string]string
    RequiredClasses []string                              // 新增，例: ["coe"]
}
```

`pg-main` `redis` `rabbitmq` 这三个基建 bundle 标记 `RequiredClasses = ["coe"]`。校验逻辑：

- 校验入口：`ReleaseService.CreateOrUpdateRelease`，跟 Phase 1 的 `ClassifyLane` 校验串行
- 校验内容：根据 lane 的 class，遍历 `app.ConfigBundles` 引用的每个 bundle，凡是 bundle 的 `RequiredClasses` 包含当前 class 的，必须 bundle.ClassOverrides[class] 非空
- 空则 reject，error message 明示哪个 bundle 缺哪个 class 的 override，让 operator 知道去哪配
...
### agent-service 非 prod lane 自动建表

agent-service 17 张 SQLAlchemy 业务表（lark_user / lark_group_chat_info / conversation_messages / bot_persona / akao_schedule / life_engine_state / glimpse_state / memory_entity / reply_style_log / fragment / abstract_memory / memory_edge / notes / schedule_revision / model_provider / model_mappings / lark_base_chat_info / ...）prod 是 ops-db submit 手建的。测试库要让它们自己建。

在 `apps/agent-service/app/main.py` lifespan 起手处加一段：

```python
if settings.lane and (settings.lane.startswith("coe-") or settings.lane.startswith("ppe-")):
    async with engine.begin() as conn:
        await conn.run_sync(Base.metadata.create_all)
```

**白名单语义而非黑名单**：只允许已知的 `coe-*` / `ppe-*` lane 触发自动建表，其他（`prod` / `blue` / 历史白名单 lane / `LANE` env 没注入时的 None）一律 fallback 到不建表。这样：

- prod / blue 启动绝不自动建表（边界由 ops-db submit 维护）
- 未来加新 lane class 必须显式更新这个判断，不会"前缀漏判默认建表"
- 比黑名单 `lane != "prod" and lane != "blue"` 更 fail-safe（None lane 不会被误判为 coe）
...
3. **paas-engine 部署 coe-* lane 后业务表 schema 跟 prod 漂移**：每次 prod schema 用 ops-db submit 加列后，下一个 coe lane 启动时 SQLAlchemy `create_all` 会发现表已存在不再 sync，新列拿不到。需要 operator 注意：每次改 prod schema 后手工 drop 测试库的对应表，让下一次 coe lane 启动重建。这是 Phase 5 真正解决（每个 coe lane 独立 schema、销毁时清理）之前的 workaround。
...
- [ ] agent-service `make deploy APP=agent-service LANE=coe-validation-1` 端到端：pod 起来连 chiwei-test-postgres、自动 create_all 17 张表、第一个业务请求不 crash
- [ ] lark-server `make deploy APP=lark-server LANE=coe-validation-1` 端到端：pod 起来 SYNCHRONIZE_DB=true 生效，TypeORM 自动 sync 13 张表
- [ ] 验证完毕清理 coe-validation-1 lane
