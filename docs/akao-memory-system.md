...
flowchart TD
    MSG["赤尾回复了一条消息"]
    MSG -->|fire-and-forget| AM

    subgraph AM["AfterthoughtManager（两阶段锁）"]
        direction TB
        P1["Phase1: debounce 5min / 15条强制flush"]
        P2["Phase2: 生成 conversation 碎片"]
        P1 --> P2
    end

    subgraph P2_DETAIL["Phase2 内部"]
        direction TB
        S1["① conversation_messages 取最近2h消息"]
        S2["② lark_user 取人名\nlark_group_chat_info 取群名"]
        S3["③ 格式化时间线\n[09:43] 原智鸿: 你好"]
        S4["④ LLM afterthought_conversation\n输入: persona + 场景 + 时间线\n场景 = 和原智鸿的私聊 / 在番剧群"]
        S5["⑤ 输出: 赤尾第一人称内心独白"]
        S1 --> S2 --> S3 --> S4 --> S5
    end
    ...
    FRAG_C["experience_fragment\ngrain=conversation\nsource_chat_id=oc_xxx\ncontent=在番剧群和阿儒聊了新番..."]

    FRAG_C -->|"03:00 CST"| DREAM_D

    subgraph DREAM_D["DreamWorker daily"]
        direction TB
        DD_IN["输入: 当天所有 conversation+glimpse 碎片\n（所有群+私聊，不分群）\n+ 最近3天 daily 碎片"]
        DD_LLM["LLM dream_daily\n睡前回想今天的一天"]
        DD_OUT["输出: grain=daily 碎片\n无 source_chat_id（跨群聚合）\n自然遗忘在此发生:\n十几条碎片 → 一篇回顾"]
        DD_IN --> DD_LLM --> DD_OUT
    end

    DREAM_D -->|"周一 04:00 CST"| DREAM_W

    subgraph DREAM_W["DreamWorker weekly"]
        direction TB
        DW_IN["输入: 最近7条 daily 碎片"]
        DW_OUT["输出: grain=weekly 碎片"]
        DW_IN --> DW_OUT
    end
    ...
    subgraph CTX["build_inner_context()"]
        direction TB
        C1["场景提示（群名/私聊对象）"]
        C2["今日基调（Schedule daily）"]
        C3["脑子里的东西（今天的碎片）"]
        C4["更远的记忆（recent daily/weekly）"]
        C5["记忆回溯引导"]
        C1 --- C2 --- C3 --- C4 --- C5
    end
