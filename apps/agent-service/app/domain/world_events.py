...
# 两端都从这里取（宪法「禁止重复定义」）。语义：这些 kind 是**被动上下文**、本身不
# 唤醒她。它的到来本身不敲门、不补敲、不打断长睡；它会在她**下次被一条客观动静
# （ambient 事件，走 EventArrived 敲门）唤醒**时，一并被读进 stimulus 里的未读项
# （list_unread_events 把信箱里所有未读项都带出来，被动的 surroundings 跟着这次
# 主动唤醒一起被读到）。
#
...
# 会让她对"该早点注意、但还没到 notify 级"的周遭变化有感知延迟（最坏延到她下次被一条
# ambient 动静唤醒时才读到）。更优方案（按变化显著度分级、或让 world 显式判这条切片要不要
# 打断长睡）待探索，详见 memory ``project_world_sense_wake_tradeoff``。
