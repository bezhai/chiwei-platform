...
#                      **她本该睡着的那段时间里别去碰她**（判的是"这一刻她本来该不该
#                      还睡着"，不是"她此刻是不是睡着"——后者是死锁：LifeState 只在她
#                      被唤醒时更新，所以"推断她在睡"恒成立、她就永远醒不了；过了她该
#                      起来的时候还没醒，反而**应该**用 idle 叫她）、**同一份没翻页的
#                      场景不逐轮机械重复判**（判据是"她这一刻该不该被打扰"）——后者
#                      堵的是唤醒风暴（world 每轮都判"仍是天然闲时刻"、一遍遍真敲门，
#                      把三个人都吵起来）。内容形态与被动 ``surroundings``
