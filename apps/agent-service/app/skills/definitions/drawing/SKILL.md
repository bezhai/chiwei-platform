...
- **风格关键词**：默认 `japanese anime style, clean lineart, flat color, cel shading`，根据场景可补充 `pastel tones` / `bright palette` 等，避免使用 `soft lighting` / `realistic` / `oil painting` 等偏写实的关键词
