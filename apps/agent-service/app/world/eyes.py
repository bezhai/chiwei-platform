...
prompt、产出朝向世界），每天带着两层感知去看：
  ...
  * **本能扫视**：开发者定的环境量必看清单（天气、日出日落、农历节气、节假日）
    ...
    什么番是世界里长出来的事实，归有意张望）。
  * **有意张望**：读最新世界阶段（知道世界走到哪、看的方向跟着阶段走）+ 读最新
    世界关注（反思留给眼睛的「想看哪」）。世界阶段 / 关注读不到时如实说空、绝不冒充
...
def _arc_evidence(arc: WorldArc | None) -> str:
    """世界阶段段落：有阶段给全文 + turned_at 时间标注；空白如实说空、绝不冒充。"""
    if arc is None:
        return "世界还没写下走到哪——世界阶段还是空白。方向上不必强求，看好环境量就够。"
...
def build_eyes_stimulus(
    *,
    ...
    """拼眼睛的两层感知 stimulus：今天日期 + 本能扫视清单 + 有意张望（世界阶段 / 关注）。

    证据风格同反思：世界阶段 / 关注全文带时间标注（眼睛要知道手里这份是多久前写的）、
    缺失如实说空。工具边界也在这里说清：番剧工具只在世界阶段 / 关注交代到时才用；
    ...
    return (
        ...
        f"必看的清单里，只在世界阶段或关注交代到追番、新番这类事时才用。专门工具没覆盖、"
        f"而世界阶段或关注让你看的事，用 search_web 兜底去查。\n\n"
...
async def run_world_eyes(*, lane: str, date: str) -> str:
    """跑一次眼睛：读世界阶段 / 关注 → 拼两层感知 stimulus → Agent 工具循环 → 返回叙述。
