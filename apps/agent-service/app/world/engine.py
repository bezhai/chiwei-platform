...
def world_loop_instruction() -> str:
    ...
    都没变）；③ 举例限定在刚起床 / 刚做完一件事 / 饭后窝着这类仍有行动余地的具体
    场景，不是抽象的"闲"概念，也不把夜晚安静本身当成休息 / 入睡提示。
    ...
    **不**是 ``next_wake_at``——那个概念已经整个删除、不该复活）：新的 idle 判断要求
    ...
    return (
        ...
        "刚做完一件事、饭后窝在沙发上这类，此刻没有别的事推着她动、但仍有自然行动"
        "余地）。**夜晚、安静、昏暗，或她上次说有点困，本身都不足以判 idle**；不要把"
        "这条通道变成休息或入睡的提示。默认 False：这份切片"
