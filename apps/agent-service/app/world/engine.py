...
def world_loop_instruction() -> str:
    ...
    第二轮调优（coe 真机暴露的两个职责边界，纯 prompt 引导、不加任何规则）：
      A. **不替三姐妹编自主行动**——world 推进的是客观环境随时间的进程（饭凉了、
         ...
         收拢碗筷"的越界。act 里捎带的、不由动作直接产生的世界断言不在「既成事实」
         之列（动作直接造成的结果仍算）：world 是世界事实的唯一来源，按自己的记录有
         来由才认。
    ...
    立刻入账、事实优先级（act 里「她做了什么」最硬、捎带的说法不在此列 / 此刻推进 >
    大纲预期 / 大纲只盯在跑没出结果的线 /
    ...
    ④ **该睡的时候别碰她 + 同一份没翻页的场景不逐轮重复触发**：若某个角色持续处于
    同一个"闲"场景（比如她一直窝在沙发上没变化，甚至她正在睡觉），只按"天然闲时刻"
    这一条标准判断会导致每轮都重新得出"这仍是天然闲时刻"的结论、每轮都传
    ``idle=True``，把三个人一遍遍真敲醒。所以 idle 判断里划了两条边界。

    这两条边界的措辞在 **idle-deadlock spec** 里做过一次重写（prod 实证：``idle_sense``
    在 20:00–06:00 之间零条、七小时零投递，world 平均 sleep 从 19.4 分钟单调涨到
    44.9 分钟）——旧措辞自己制造了两个病：

      * 边界①旧判据是"（时间外推）推断出来就是在睡 → ``idle`` 绝不能填 True"，而
        :class:`LifeState` **只在她被唤醒时才更新**：上次观测在睡 → 外推还是在睡 →
        禁止叫 → 永不唤醒 → 状态永不更新。这是个不需要模型犯错就会发生的**闭环
        死锁**。现在判据换成"这一刻她**本来该不该**还睡着"：落在她本该睡着的时段
        → 别碰（``idle`` 仍绝不能填 True）；**过了她该起来的时候她却还没醒 → 可以
        用 ``idle=True`` 叫她**。world 做得了这个判断——每个角色一天大致怎么过写在
        它的 system prompt（``world_deliberate``）里，加上 ``observed_at`` 和
        【现实此刻】就够；这段指令只**引用**这份既有认识、不重述设定（USER 层不描述
        世界设定，见 :func:`world_loop_instruction` 上方的既有约束）。措辞里还直接
        点破了自锁本身（"那份状态只有在她醒着的时候才会更新"），免得模型自己又推回
        旧循环。
      * 旧措辞里"（她真的换了个状态、**或者隔了足够久**这又成了一次新的闲时刻）"把
        **把 sleep 排长**明码标价成重新解锁 ``idle`` 许可的手段——模型想 idle 却被
        边界挡住时，最直接的合法解就是睡更久；sleep 时长的单调上涨就是被这句奖励
        出来的。整句删除。
      * 旧措辞里"世界大约每 30 分钟推一轮……这正是当年不得不把 sense 整体改成被动
        通道的那次事故根源"是错误自述：代码里没有 30 分钟这个数（心跳
        ``WORLD_HEARTBEAT_SECONDS`` 是 600s、``sleep`` 60–3600s 全由模型自定），
        它既把尺度讲错、又把 30 分钟标成事故量级（于是安全解自然是排 >30 分钟）；
        它引用的那次事故里的自排睡眠机制早已整体删除，事故标签同样过时。"别机械地
        每轮都判一次"这层意思保留，但不再给量化尺度和事故标签。

    边界②**不删只温和化**：``derive_idle_sense_event_id`` 带 ``round_id``（每轮是
    不同事件、不会被幂等吞掉）、``idle`` 默认 ``False`` 拦不住模型每轮显式传 True、
    ``PASSIVE_EVENT_KINDS`` 只含 ``surroundings`` 不管 ``idle_sense``——它是当前
    **唯一**拦住"每轮把三个人都叫醒"的东西，删掉会直接复现事故。温和化 = 判据从
    "这个场景跟上轮比变没变"换成"她这一刻该不该被打扰"，保留抑制力，但不再要求她
    必须先换个状态才配被叫。
    ...
    **不**是 ``next_wake_at``——那个概念已经整个删除、不该复活）：新的 idle 判断要求
    ...
    return (
        ...
        "**反映她已经做了的事** ✓ —— 你收到的角色动作（act）里「她做了什么」是她**已经做了**的既成事实，"
        "把它的客观结果体现进世界（她去了厨房 → 厨房有了动静和她的身影），这是反映、不是"
        "替她决定；**预先替她编她还没做的行动** ✗ —— 她还在吃饭、还没醒，你就**绝不能**"
        "写「她起身收拾了 / 她去写作业了」这种她没做的自主行动，那是替她做主、越了界。"
        "**还有第三样要分清：她的动作不等于她的说法。**她做的动作本身直接造成的结果，仍算她"
        "做的事（她把饭端上桌 → 桌上有了饭；她把窗推开 → 窗开着），照常反映。但动作描述里若"
        "捎带了不由这个动作直接产生、要靠世界另有来由才成立的断言——别人身体怎么样、来了个"
        "电话、外面出了什么事、测出了什么结果——那不是她做的事，是这个世界的事实，而世界的"
        "事实只有你能确立：回头看你自己的记录，有来由就落实，没来由就不把它写成事实、照世界"
        "原本的样子接着写（电话没响过、外面什么也没发生），她的说法留给她自己。几个人互相说"
        "同一件没来由的事，也不会让它变成真的。"
        "\n"
        "**真实时间一直在往前流逝**：你记得"
        ...
        "自锁，绝不能这样判断）。\n"
        "  **两条边界，绝不能踩**：一是**该睡的时候在睡，就别去碰她**——但要判的不是"
        "「她此刻是不是睡着的」，而是**这一刻她本来就该睡着，还是早该醒了**。每个角色"
        "一天里大致什么时候该躺下、什么时候早该起来，你对这个世界的既有认识里已经写着"
        "了；把它和【现实此刻】一对，就能分清两种「她还在睡」：**落在她本该睡着的那段"
        "时间里** —— 那是她该睡满的觉，idle 绝不能填 True，这一刻根本不该碰她；"
        "**现实此刻早已过了她该起来的时候、她却还没醒** —— 那不是该睡满的觉，是她被"
        "落在了昨天，这时候用 idle=True 把「天已经亮了、家里已经在动」这份客观场景递"
        "到她跟前，恰恰是该做的事。**别倒过来推**：「她上次被观测到时在睡，所以推断她"
        "现在还在睡，所以不叫她」——她那份状态只有在她醒着的时候才会更新，拿「她一直"
        "显示在睡」当「她该继续睡」的证据，是个会把她永远关在里面的自锁；要看的是现实"
        "此刻落在她一天的哪一段。真有必须惊动她的理由（比如失火、地震这类），那是 "
        "notify 该管的真动静，走它自己「谁在场谁被卷进去」的逻辑，不归 idle 插手。"
        "二是**同一份没翻页的场景，不逐轮机械重复判 True**——你是一个连续推演的脑子，"
        "记得自己上一轮做过什么：如果你上一轮已经因为这同一份「她窝在沙发上」的场景把"
        " idle 判成过 True、把她叫起来过了，这一轮先问一句**她这一刻该不该被打扰**"
        "——刚递过去一次、这中间世界也没给她递来什么新的，那就让她自己待着，别照本"
        "宣科再判一次 True。反过来，这一刻她那边确实到了值得她抬头看一眼的时候（天光"
        "变了、家里开始有动静、她该起来的时候到了），就不必等她先换个状态才算数。别"
        "机械地每轮都对每个人判一次 True（那是一次次真把她吵醒），也别因为怕吵到她就"
        "一次都不判——两头都不是你要的。\n"
        ...
        "时间外推准则、以及「她本该睡着的时段别去碰她」「同一份没翻页的场景不逐轮重复"
        "判」这两条边界，不会重现旧的自锁）。除了这一个划定清楚的例外，其余所有场景里：时间到了、下课了、"
        ...
        "③ **事实之间有优先级**：你收到的 act 里「她做了什么」最硬，你不能否认她做过、"
        "只**推演**它带来的客观后果；act 和自述里捎带的、不由动作直接产生的世界断言（别人的"
        "身体、外面发生了什么事、测出的结果）不在这个「最硬」之列，它们由你按自己的记录确立——有来由"
        "就认，没来由就不认、写出真实的样子；「这轮 / detail 体现的现实」高于「大纲里那"
        ...
        "可以给（网购下单了这条线 → 两三天后快递到了，给；衣服洗了晾在阳台这条线 → 傍晚"
        "收进来是干的，给）；没有任何线铺着、凭空硬造的遭遇，不给（好端端在家突然停电、"
        "突然有陌生人上门，不给）。别为了热闹即兴硬造。\n"
        "⑤ **全程只确立客观**（包括落到身体上的生理客观，比如她淋了雨衣服湿透、在太阳底下走了"
        "一下午皮肤晒红、从早上到现在没吃过东西），**绝不碰角色的主观承受与应对**（累不累、饿"
        "不饿、什么心情、硬撑还是去躺会儿——那是角色自己的事；她什么时候起、要不要躺下是她的"
        "自主行动，同样不由你写）。她自己报的感受（头晕、没力气），认她的、不否认——那是她此刻"
        "的主观体验；但感受不自动确立病因、体征、测出的结果这些客观身体状态，那些只从你记录里"
        "的来由推：没有来由就不新增客观结论，也不反过来否认她的感受。哪怕她自己或姐妹嘴上断言"
        "她身体出了什么状况，客观身体状态仍由你按来由写。\n"
...
def _sisters_section(states: list[tuple[str, LifeState | None]]) -> str:
    """把三姐妹此刻各自的自述渲染成喂给 world 的一段文本——每人带上**当前状态 + 观测时刻**。
    自述只确立她自己在做什么；里面捎带的世界事实由 world 按自己的记录确立。
    ...
    **``observed_at`` 的复活是范围很窄的一次例外（life-idle-wake-via-sense Task 1
    ...
    （是不是天然的闲时刻 + 她本该睡着的时段别碰她 + 同一份没翻页的场景不逐轮重复判，见
    ...
    body = "\n".join(lines)
    return (
        "下面是三姐妹此刻各自的自述——每人自己记下的此刻在哪、在干嘛、这份状态是什么时候"
        "观测到的。这首先是给你做客观叙述对齐用的（她在上课就别说她在街上）；但自述是她"
        "自己的视角：她在做什么、她自己的感受，认她的；她自述里若说到世界的事实——别人的身体、"
        "外面发生了什么、测出了什么——以你自己的记录为准。观测时刻这份"
...
def _world_loop_messages(
    ...
    **当前状态 + 观测时刻**）。**必传、无默认值**：拼在【现实此刻】之后，让 world 的
    ...
    18:30 写「更深的蓝灰」，把天黑提前了 1.5–2 小时——【现实此刻】只给裸时刻，而循环
    指令反复拿「天色暗下来」当时间推进的范例，模型只能用「傍晚≈天黑」的通用先验渲染
    黄昏。补的是**当天的客观天文事实**，不是「几点之后算晚上」的规则（赤尾宪法：
    不用代码替 agent 做世界内容决策）。坐标没配 / 算不出时传空串、这一行就只有裸
    时刻（如实不拼，**绝不编一个日落时刻**）。
    ...
    act_section = (
        ...
        "这里列的是她们各自做了什么；动作里捎带的对世界事实的说法，按你的规矩由你确立。）\n"
        f"{act_batch_text}\n\n"
        if act_batch_text
        else ""
    )
...
async def _run_world_round(tick: WorldTick, *, lane: str) -> None:
    ...
    # 今天的日照锚点（idle-deadlock Task 2）：算当天日出 / 日落贴在【现实此刻】后面，
    # 让 world 渲染黄昏时有当天的客观事实可依，不再用「傍晚≈天黑」的通用先验把天黑
    # 提前 1.5–2 小时。坐标走 Dynamic Config，没配 / 配脏 / 算不出返回空串（如实不拼、
    # 绝不编造）。这里只给客观天文事实，不给任何「几点之后算晚上」的判断（赤尾宪法）。
