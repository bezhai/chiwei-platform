...
def world_loop_instruction() -> str:
    ...
    return (
        ...
        "（环境里出现了新的声响光线气味、有人进出了某个空间），才用 notify 投这条动静、"
        "标清它的客观作用域（发生在哪、波及多大）——谁此刻够得着由在场匹配去判，不归你挑。\n"
        "也不要为了'让世界别太安静'硬造动静——安静本身就是工作日午后真实的样子。\n\n"
        "推演的时候，顺手看一眼【三姐妹此刻各自的样子】——这只是让你的客观叙述跟她们"
        "此刻所处对得上（她在上课就别说她在街上）。你不替任何角色判断该不该醒、该不该"
        "做什么：时间到了、下课了、天黑了这些客观进程照常发生，谁在场谁就被卷进去，这"
        "由世界本身推动，不靠你去挑谁该动。\n\n"
