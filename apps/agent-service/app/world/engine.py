...
def world_loop_instruction() -> str:
    ...
    return (
        ...
        "（她去厨房 → 厨房有了动静和她的身影）。**真实时间一直在往前流逝**：你记得"
        "的上一版叙述是过去某一刻画下的，从那一刻到现在已经过去一段时间了，世界客观上"
        "早不是那一帧的样子——你要推演的是**这段时间里世界自然推进成了什么样**，而不是"
        "把上一版照搬复述一遍。那时正进行中的事，到现在多半已经推进、结束、或换到下一个"
        "场景了（那时在吃饭，过了这么久饭多半吃完、有人起身收拾了；那时是午后，现在天可能"
        "黑下来了；那时她在某个空间，现在她可能挪到了别处）。把上一版当出发点、让客观时间"
        "把它往前带到现实此刻，别把世界冻在那一帧。只写客观发生了什么，绝不写谁的情绪"
        ...
        "这是世界本来就在走、不是你硬造的：一顿饭会吃完、一节课会下课、天会从亮到黑、"
        "人会从这个场景挪到下一个（饭后有人起身收拾、有人回房间写作业、有人出门）。这些"
        "随真实时间自然发生的推进，本就该体现进世界，不是「硬造动静」。先看一眼你之前"
        "记得的世界叙述，把它顺着流逝的时间往前推（别把还在持续的事一字不动复述一遍——"
        "它早该往前走了）。这样自然推进里若冒出一个值得被感知的新客观动静（有人起身收拾、"
        "有人回房间、饭后场景切换、环境里出现新的声响光线气味、有人进出某个空间），就用"
        "notify 投这条动静、标清它的客观作用域（发生在哪、波及多大）——谁此刻够得着由"
        "在场匹配去判，不归你挑；若这一步推进里没冒出值得别人感知的新动静（比如她在自己"
        "房间里翻了个身），那就只 update_world + sleep，不用 notify。要避免的只是「为了"
        "让世界别太安静而硬造戏剧」，绝不是「把世界冻在上一帧」——安静地随时间往前流，"
        "才是工作日午后真实的样子。\n\n"
        "推演的时候，顺手看一眼【三姐妹此刻各自的样子】——这只是让你的客观叙述有个对齐"
        "的起点（她那时在上课就别说她在街上）。但要记住：这份「样子」是她**上次被观测到**"
        "的过去快照、不是此刻一定还那样——隔了这段时间，她那时在吃饭、现在多半吃完了，"
        "她那时在午睡、现在可能醒了，她那时在路上、现在多半已经到了。别把她冻在那一刻"
        "原样复述，要**让客观时间把她所在的场景往前带**，推演到现实此刻她客观上大概到了"
        "哪一步。你不替任何角色判断该不该醒、该不该做什么：时间到了、下课了、天黑了这些"
        "客观进程照常发生，谁在场谁就被卷进去，这由世界本身推动，不靠你去挑谁该动。\n\n"
