...
**续写（``WORLD_TOOLS``，四件）**——顺着流往前推，不质疑前提：
  ...
  * :func:`sense`        —— world 五官（1C Task 2）：给**单个**角色投她此刻的周遭
    ...
    闲时刻（刚起床 / 刚做完一件事 / 饭后窝着，对她上次已知状态做时间外推）时把
    ``idle=True``
...
async def sense(recipient: str, surroundings: str, idle: bool = False) -> str:
    ...
    **不该**包含她够不着的别处（厨房在发生什么、客厅有谁）。逐角色分别推演、各投各的
    ...
    Args:
        ...
        idle: 你判断这一刻是不是 recipient 天然的闲时刻（刚起床、刚做完一件事、饭后
            窝在沙发上这类，此刻没有别的事推着她动、但仍有自然行动余地）。夜晚、安静、
            昏暗，或她上次说有点困，本身都不足以判 idle；不要把这条通道变成休息或入睡
            的提示。默认 False——这份切片安安静静躺进她的
