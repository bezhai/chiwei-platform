...
**续写（``WORLD_TOOLS``，四件）**——顺着流往前推，不质疑前提：
  ...
  * :func:`notify`       —— world 产一条客观动静（``observation``，自然语言客观描述）
    + 标它的**客观作用域**（``scope``：发生在哪、波及多大）。谁收到不由 world 挑：
    工具调一道轻量在场匹配（:func:`app.world.presence_match.match_present_personas`，
    纯模型判断，拿作用域对每个角色此刻客观位置去判）拿在场角色，对每个在场角色包
    ``deliver_event`` 投进其信箱（kind=ambient、source="world"）。Task 3：把"挑收件人"
    从 world 切走——world 只标作用域（客观描述的一部分），在场由独立判断决定。
  * :func:`sense`        —— world 五官（1C Task 2）：给**单个**角色投她此刻的周遭
    ...
    工具体注释 + memory project_world_sense_wake_tradeoff）。**``idle`` 参数**
    （life-idle-wake-via-sense Task 1）：world 判断这次投递的场景是 recipient 天然的
    闲时刻（刚起床 / 睡前 / 饭后窝着，对她上次已知状态做时间外推）时把 ``idle=True``
    传给 sense，工具改投 kind=idle_sense（**不在** ``PASSIVE_EVENT_KINDS`` 里）——
    真正敲门唤醒她，把这份客观场景如实递给她自己的模型去决定要不要做点什么。默认
    ``idle=False`` 保持原被动行为不变。
...
赤尾设计宪法：
  ...
  * 谁收到不由 world 主观挑：world 标客观作用域，在场由
    :func:`app.world.presence_match.match_present_personas` 纯模型判断（拿作用域对每个
    角色客观位置去判），工具 / 在场匹配都不查表、不机械匹配房间、不加距离阈值 /
    位置枚举 / if 分支。
...
# 闲时刻主动周遭切片（sense idle=True）的 event_id 派生命名空间：与被动
# ``_SURROUNDINGS_EVENT_ID_NS`` 分开，让同样文字的「被动周遭切片」与「主动周遭切片」
# 派生出不同 id、不在 deliver_event 幂等里互相吞掉（两类 kind 不同、语义不同——一个
# 不唤醒、一个真正唤醒她）。
_IDLE_SENSE_EVENT_ID_NS = uuid.UUID("1e4f6a8c-3d2b-4c5e-9f7a-2b1d0e3c4f5a")
...
def derive_event_id(*, lane: str, observation: str, round_id: str) -> str:
    ...
    同一 observation 投多个在场角色共享这一 id（persona 不同自然键不同，
    不冲突）。不同 observation（不同的动静）派生不同 id。谁收到由在场匹配判
    （:func:`app.world.presence_match.match_present_personas`），不靠房间锚点。
    """
...
def derive_idle_sense_event_id(
    *, lane: str, recipient: str, surroundings: str, round_id: str
) -> str:
    """确定性派生一份闲时刻主动周遭切片的 id：同输入同 id（同 :func:`derive_surroundings_event_id`）。

    与 :func:`derive_surroundings_event_id` 派生源完全同形（lane / recipient /
    surroundings / round_id），唯一区别是**独立命名空间**（``_IDLE_SENSE_EVENT_ID_NS``）
    ——即便 world 给同一个 recipient 在同一轮写出完全相同的文字，一次 ``idle=False``
    一次 ``idle=True``，也必须派生出不同 event_id：两者是不同语义的 event（一个不
    唤醒、一个真正唤醒她），不能在 ``deliver_event`` 幂等里让后写入的那次被误判成
    对前一次的重复投递而吞掉。
    """
    return uuid.uuid5(
        _IDLE_SENSE_EVENT_ID_NS,
        f"{lane}\x1f{recipient}\x1f{surroundings}\x1f{round_id}",
    ).hex
...
# 还没活过一轮（无 LifeState / current_state 空）的角色喂给在场匹配的位置占位文本：
# 如实说「还不知道她此刻在哪」，把「她在不在场」这个不确定交给模型判（赤尾宪法：不确定
# 性留给模型、不加规则消除）。**不能把她整个排除**——冷启动时谁都还没活过一轮，排除了
# 第一条客观动静就永远到不了任何人、世界起不来（自锁）。
_UNKNOWN_LOCATION = "（还不知道她此刻在哪——她还没活过一轮、没有可读的此刻状态。）"


async def _current_persona_locations(*, lane: str) -> dict[str, str]:
    """读每个角色此刻的客观位置（在场匹配的候选面）：``persona_id -> 位置自然语言``。

    位置不是结构化字段——它融在 life 的 ``current_state`` 自然语言里（原设计选择，
    不为在场匹配加结构化 location 字段）。**每个角色都进候选**：有 LifeState 就喂她
    的 ``current_state`` 原文，读不到（她还没活过一轮）就喂 :data:`_UNKNOWN_LOCATION`
    占位、把「她在不在场」交给模型判——绝不在代码里把无状态的角色排除掉（冷启动谁都
    还没活过一轮，排除了第一条客观动静就永远到不了任何人、世界起不来）。用本轮 tick
    的 lane（泳道隔离命门，同 WorldState）。
    """
    locations: dict[str, str] = {}
    for persona_id in await list_all_persona_ids():
        state = await find_life_state(lane=lane, persona_id=persona_id)
        if state is not None and state.current_state:
            locations[persona_id] = state.current_state
        else:
            locations[persona_id] = _UNKNOWN_LOCATION
    return locations
...
async def update_world(detail: str) -> str:
    ...
    进来（她去厨房 → 厨房有了动静和她的身影）。动作本身直接造成的结果（把饭端上桌
    → 桌上有饭）仍是动作的一部分、照常反映；动作里捎带的、不由动作直接产生的世界
    断言（别人的身体状况、来电、外面出的事、测出的结果）由你按自己的记录确立：有
    来由就落实，没来由就不写成事实，她的说法留在她那里。detail 只写客观发生了什么，
    绝不写谁的情绪 / 心情 / 主观解读——那是角色自己的事。
...
async def update_outline(narrative: str) -> str:
    ...
    边界（别让大纲往别处漂）：
      ...
      * 不写主观感受（累不累、什么心情、硬撑还是去歇会儿）——那是角色自己的事，你只
        确立客观、绝不替角色决定她怎么应对。
...
@tool
@tool_error("投递动静失败")
async def notify(scope: str, observation: str) -> str:
    """产出一条客观动静、标上它的**客观作用域**——谁收到由在场匹配决定，不是你挑。

    observation 必须是客观可感的形态（感官投影），比如"厨房飘来煎蛋和咖啡的香味"
    "玄关传来开关门的声音""晌午的光斜照进房间"。绝不写情绪、心情、主观解读、
    建议或指令。

    scope 是这条动静的**客观作用域**——它客观上发生在哪、波及多大范围：是全场都
    听 / 看得到的广播（"下课铃响，整座学校都听得到"），还是冲着某个具体空间 / 人去的
    （"屋里厨房有人朝餐桌方向喊吃饭——声音只在屋里传得到"）。这是你对这条动静的**客观
    描述的一部分**（它发生在哪、传到多远），**不是**"我要推给谁"的决定——谁此刻在那个
    范围里、够得着这条动静，由后面一道在场匹配按每个角色客观在哪去判，不归你挑。所以
    你只管把这条动静发生在哪、波及多大说清楚，别去想"该给谁"。

    Args:
        scope: 这条动静的客观作用域（自然语言：发生在哪、广播还是指向某个空间 / 人、
            波及多大范围）。
    ...
    # 在场匹配（替换旧的 world 主观挑 recipients）：拿这条动静的客观作用域，对一下每个
    # 角色此刻客观在哪（从 life 的 current_state 读），谁在场谁收到。这道判断只看「动静
    # 发生在哪 + 谁在那」、不看「谁该不该被叫醒」——是纯模型判断（match_present_personas
    # 内是一次离线 LLM 结构化判定），代码侧无任何确定性匹配规则（赤尾宪法）。
    persona_locations = await _current_persona_locations(lane=lane)
    recipients = await match_present_personas(
        scope=scope,
        persona_locations=persona_locations,
        trace_session_id=get_context().session_id,
    )
...
@tool
@tool_error("投递周遭切片失败")
async def sense(recipient: str, surroundings: str, idle: bool = False) -> str:
    ...
    **不该**包含她够不着的别处（厨房在发生什么、客厅有谁）。逐角色分别推演、各投各的
    ...
    Args:
        ...
        idle: 你判断这一刻是不是 recipient 天然的闲时刻（刚起床、睡前、饭后窝在沙发
            上这类，此刻没有别的事推着她动）。默认 False——这份切片安安静静躺进她的
            信箱，不打扰她，她下次自己醒来时才会读到。你判断这一刻确实是天然闲时刻
            时填 True——这次投递会真正把她叫醒，把这份客观场景如实递给她，让她自己
            的脑子决定要不要做点什么（刷手机、翻翻书，或者什么都不做）。判断依据是
            时间外推、不是等她先做点什么：跟你推演 update_world 用的是同一种功夫——
            看她上次被观测到的状态 + 这中间流逝的真实时间，推断这段时间大概率没有
            新动静、她大概率还在原来那种安静状态里，而不是坐等她自己先冒出一个新
            动作来证明她真的闲。
    ...
    # 主动 / 被动只在这一处分岔：kind 不同（EVENT_KIND_IDLE_SENSE vs
    # EVENT_KIND_SURROUNDINGS），event_id 命名空间跟着 kind 走（各自的 derive 函数，
    # 同样文字也不互相幂等吞掉）。往下 deliver_event 调用完全同形——wake 判定统一
    # 交给 kind 是否落在 PASSIVE_EVENT_KINDS 里，这里不再需要（也不该）传独立的 wake
    # 参数（历史教训：独立 wake 参数只挡即时敲门、挡不住补敲对账，是不完整抽象，见
    # deliver_event docstring）。
    if idle:
        kind = EVENT_KIND_IDLE_SENSE
        event_id = derive_idle_sense_event_id(
    ...
    occurred_at = cst_time.now_cst_iso()
    try:
        # kind=EVENT_KIND_SURROUNDINGS 在 PASSIVE_EVENT_KINDS 里——周遭切片走**被动
        # 通道**：deliver_event 按 kind 判断不敲门唤醒（通道分离的权宜修复 v2，prod
        # 节奏失控）。world ~30 分钟推一轮、每轮 sense 给三姐妹各投一条周遭切片；若走
        # 唤醒通道（设计上永远放行、不走"到点才醒"的 gate）会把自排睡着的姐妹全敲醒、
        # 自排睡眠系统性睡不满。改为被动：她下次自己醒来（self-wake 到点）时
        # list_unread 自然读到最新周遭。
        #
        # kind=EVENT_KIND_IDLE_SENSE（life-idle-wake-via-sense Task 1）**不在**
        # PASSIVE_EVENT_KINDS 里，走**主动通道**：world 判断这一刻是天然闲时刻时，
        # 这次投递该真正唤醒她——deliver_event 按 kind 判断，跟 notify / npc_visit /
        # 手机消息一样照常敲门；world engine 每轮的补敲对账
        # （list_personas_with_unread）同样按 kind 判断，两条路径天然口径一致，不需要
        # 给它们加任何新代码。被动语义统一落在 kind 上（即时敲门 + renotify 补敲对账
        # 都读 PASSIVE_EVENT_KINDS），所以这里不传独立的 wake 参数。权宜解的二分粗 /
        # 感知延迟取舍见 memory project_world_sense_wake_tradeoff。
        await deliver_event(
            lane=lane,
        ...
        logger.warning(
            "world sense 投递给 %s 失败（lane=%s event=%s idle=%s）",
            recipient,
            lane,
            event_id,
            idle,
    ...
    if idle:
        return f"已把此刻的周遭切片投给 {recipient}，真正唤醒了她"
