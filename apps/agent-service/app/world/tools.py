...
**反思（``WORLD_REFLECT_TOOLS``，两件）**——对表翻页，质疑前提：
  ...
  * :func:`update_attention` —— 重写「世界当前想看哪」（留给眼睛的关注，与长弧
    语义不混放：长弧写走到哪、关注写想看哪）。契约与 update_arc 完全同款：
...
async def update_attention(narrative: str) -> str:
    ...
    长弧写「世界走到哪」，关注写「想看哪」：在等什么消息、想确认什么事、什么时候
...
# sense / sleep；反过来续写与眼睛也无手碰长弧和关注（姿态物理隔离）。
