...
**反思（``WORLD_REFLECT_TOOLS``，一件）**——对表翻页，质疑前提：
  ...
  * :func:`update_arc`   —— 重写「世界的长弧走到哪」（与 update_world 同族、分
    ...
    续写姿态发现不了「页翻了」（coe 实证：长弧 v1 把过期底色结晶了进去），翻页
    归无会话、每日一次、对表现判的反思独占——续写无手碰长弧、反思无手碰
...
# 反思工具集（一件）：对表翻页。反思环节（app.world.reflection）每日一次、无会话，
# 工具只有 update_arc——反思无手碰 detail / notify / sense / sleep。
WORLD_REFLECT_TOOLS = [update_arc]
