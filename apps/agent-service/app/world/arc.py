"""世界阶段快照 — 活的世界（时间常数分层）的慢层.

world 的自产状态分两层钟、两张表：``WorldState`` 写「此刻的世界是什么样」（每轮
都可能重写，明天就过时）；``WorldArc`` 写「跨周月仍然成立的世界进展」——「世界
走到哪个阶段」的自然语言全文快照，只在 world 推演判断到翻页级转变（考完 / 放榜 /
搬家 / 换季）时整篇重写。判据一句话：这句话下周还成立吗？成立才配进世界阶段。

每次更新都是整篇重写（翻过去的页不留在世界阶段里：高考结束**取代**备考，不是排在
...
class WorldArc(Data):
    ...
    自然键 ``lane``（每个泳道一条世界阶段）。``narrative`` 是整篇重写的世界阶段全文——
    ...
    ``version`` 让同一 lane 的多版世界阶段 append-only 保留历史、读最新一版。
    """

    lane: Annotated[str, Key]
    narrative: str   # 世界阶段全文（world 整篇重写的自然语言）
...
async def write_world_arc(*, lane: str, narrative: str, turned_at: str) -> None:
    """append 一版世界阶段（world 推演判断到翻页级转变时整篇重写）。
...
async def read_world_arc(*, lane: str) -> WorldArc | None:
    """读某泳道最新一版世界阶段，没有返回 None（冷启动：世界阶段还是空白）。"""
