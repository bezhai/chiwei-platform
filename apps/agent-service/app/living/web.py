...
**一个字都不落库。** 这两只手的结果只进**本轮**的上下文、影响她这一轮的输出，下一轮就
没了 —— 跟真人刷完就忘是一回事。所以这个模块里没有 ``Data``、没有游标、没有水位。她想
把什么留到下一轮，用她自己那份"心里挂着没了结的事"
...
def _note(hand: str, what: str) -> None:
    ...
    **这是事后唯一查得到的地方**：这两只手一个字都不落库（结果只活在本轮的上下文
    里），而 langfuse 会系统性丢 trace（见 :mod:`app.agent.trace`）。在打出去之前
    记，这一趟挂了也留得下痕。顺带，读 moment 身份本身就是 fail-fast：没绑进 moment
    的调用当场就报错，不会安静地照跑。
    """
