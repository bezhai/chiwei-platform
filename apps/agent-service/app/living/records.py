...
def esc(s: str | None) -> str:
    ...
    **哪些要过、哪些不要：判据是"逐字通道"，不是"谁写的"。**
    ...
    的标题 / 链接 / 摘要、图片站的标题。**world 从六个真实数据源抄进世界的那些字**
    （天气、番名）也过这一道：中间虽然隔着一个模型，但转写正是它最容易被劝着做的事，
    而 ``Happening.content`` 上这一道没有代价。

    转写不止 ``Happening.content`` 一条：**world 写进地方文档的正文和它给文档起的
    名字**（:func:`app.living.moment.arriving_at`）走的是同一条通道，而且那条绕开了
    这两处 —— 正文不经过任何一张表，直接从文件系统进她这一轮。那儿的代价也量过：
    地方文档是自然语言，四个字节之外一个不动。
