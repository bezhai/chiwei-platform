...
**哪一轮都能调，不绑在"换事情"上。** 「是否换事」不等于「是否记住」：有人跟她说了
句要紧的话，她手上的书没放下（那一轮答「继续」），但她记住了。绑在换事情上的话，这
条感知在游标推进之后就永久消失——上下文里那一段 4 小时就裁掉，状态里「你刚做过、说过」
那一段只有她**自己**说做的，别人说的话不在里面。

**已知限制（不修，coe 上观察）：线头的身份是那句话的原文。** 所以

  * 她换个说法重述同一件事 → 算成一条**新**线头，旧的那条因为没被列出来而关掉；
  * 她某一轮漏抄了一条 → 那条就此关掉，跟她真的了结了它分不出来；
  * 关掉之后原句一字不差地再出现 → 复活，出处停在第一次那一轮，中间那段断档在数据
    上看不出来（读起来像"从头到尾一直挂着"）。
...
``opened_moment_id`` 永远停在第一次写下的那一轮。这是"跨多轮没被遗忘、而且指得出
是从哪一轮带过来的"这条验收的全部依据：清单上每一条都自带出处。

**关掉的又被列出来 = 复活，出处不变。** 她重新惦记起同一件事，说明它从最早那一轮
...
拆开、:func:`format_entry` 拼回），而快照渲染回给她的就是拼回来的那个形状——她下一轮
...
# 年月日 + 时分，北京时间。她这一轮的第一行就是 ``现在 2026-07-25 周六 14:35 CST``
...
class LooseEnd(Data):
    """她心上挂着的一件事：什么事、从哪一轮起挂着、了结了没有。

    自然键 ``(lane, persona_id, thread_id)``；``thread_id`` 从 ``what`` 派生
    （:func:`derive_thread_id`），所以重写清单不会堆出重复行。

    ``opened_moment_id`` 是**第一次**把它写进心上的那一轮，之后再怎么重写都不变。
    ``closed_moment_id`` 是关掉它的那一轮。这两列不是日志：验收要对每一条"跨轮
...
def parse_entry(raw: str) -> tuple[str, datetime | None]:
    ...
    **写不成的一律当场炸，不静默降级。** 三种写法各有各的静默毒性：
      ...
      * **时刻写了、那件事忘了写**（「[2026-07-25 15:00]」）—— 这条最危险：它返回的
        ...
        个空行"合流、被跳过。整份重写之下，她这一轮要是只列了这么一条，**她心里挂着
        的会被全部关掉，而工具还回她一句成功**。她漏写的只是那件事本身，代价却是一
        整份清单。

    炸出去的代价只是这次醒来她重写一遍——报错原样喂回给她（``@tool_error``），同一轮里
    就能改对。
...
def format_entry(what: str, due_at: datetime | None) -> str:
    """拼成她下一轮该照抄的那一行 —— :func:`parse_entry` 的反面。

    快照和 ``keep_in_mind`` 的确认都用它，所以她眼前见到的形状永远就是她该写回来的
    形状。这比在工具描述里讲一遍格式管用得多：她每一轮都在读这个形状。
    """
    if due_at is None:
        return what
...
async def rewrite_loose_ends(
    ...
    **"她敲了个空行"和"她写了一半"是两种空，别合流。** 纯空白的一行直接忽略——里面
    ...
    合流的代价是一整份清单：整份重写之下，她这一轮要是只列了这么一条，跳过它就等于
    ...
    **整份先解析完再动库**：一条写坏了，这一份整个不落，她上一份清单原样还在。
    ...
    返回值让调用方（这一轮）直接报得出"轮末还挂着几件"，不用再查一次库。
    """
...
async def _keep_open(
    ...
    **时刻没变时必须是 no-op**：她每十分钟重列一遍整份清单，一动就 append 的话，一条
    ...
    件事，说明它从最早那一轮起就一直在她心上；改个时间更不是新开一件事。换一个新出处
