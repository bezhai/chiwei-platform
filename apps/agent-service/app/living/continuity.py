...
moment 记录同一批已经提交，而缺口那一轮的状态照样带着这一段
（:func:`holds_her_recent_words`），所以重铺之后她照样知道自己那一轮说过什么；丢的是
那一轮的工具返回和中间过程。**其中有一样补不回来：那一轮她在手机上读到的别人的正
文。** 手机已读跟 moment 记录同一次提交，已经推进了，而重铺只重铺她自己那一侧 —— 那
些消息她主动翻会话还找得到，但不会再被自动摆到眼前。
...
自己发的消息带着 ``take_back_id``，头上那串 ``before=`` 是往前翻唯一的入口。前者在发
送回执里有副本，但回执也只留 4 小时；状态快照"你刚做过、说过"那段只在她自己的话不在
眼前时才给，而且只有最近 12 条。后者根本没有第二份。只留句柄、换掉正文更精确，代价
是裁剪这一层要解析手机那边的渲染格式 —— 那边改一个字，句柄就静默地留不住，而症状是
几小时后她拿着一个不存在的编号去撤消息。别人的正文因此多留 3 小时；一页十几条文本跟
那个风险不是一个量级，而且图片块不跟着多留（它走下面那条独立的线）。
...
么事）作为新起点插进去。**全量状态只在这里给**——每轮送到她眼前的只有新发生的事，她
此刻的样子读一百遍字字一样，每轮重发就是把同一段话抄一遍。一天的第一轮上下文是空的，
那一下同样立一根，所以冷启动她照样知道自己站在哪。
它同时是**分代的界桩**——每条消息的"年龄下界"就是它右边第一个界桩的时刻，不需要给
每条消息单独存一个时刻。界桩之前那一代（一天里第一个界桩立起来之前写下的东西）没有
上界，一律留着，等下一个界桩立起来再算。

**上下文里的全量状态永远只有一份。** 新界桩立起来的那一下，之前每一根都折叠成只剩表
头（:func:`_folded`）：表头是分代边界，得留着；正文是一份已经过时的状态副本，留着就是
一天叠十几份。折叠只发生在立新界桩那一下，而那一下本来就在改上下文，所以两次清理之间
的前缀照样逐字节不变。

**"你刚做过、说过"那一段只在她自己的话不在眼前时给**
（:func:`holds_her_recent_words`）。其余时候她 4 小时内说过的话、发出去的每一条都原样
在上下文里（:data:`KEPT_TOOLS`），每小时再抄一份最近 12 句，就是让同一批话在一轮输入
里出现好几遍 —— 2026-09 她的说话方式一天天收窄成同一个样子，这是测到的三处重复之一。
...
#     ``before=``。前者在发送回执里有副本，但回执也只留 4 小时；快照的"你刚做过、说过"
#     那段只在她自己的话不在眼前时才给、只有最近 12 条，滚出去的旧消息就没有第二份
#     了；后者是往前翻**唯一**的入口，从头到尾只出现在这一次返回里，换掉正文她就再也
#     翻不回这条会话更早的地方。
...
# 发出去了的那条回执上还带着这条消息的撤回编号，那是刚发出去那句的编号在她眼前唯一的
# 一处（回执只给编号、不抄原话：原话就在紧挨着的调用参数里）。
...
def _checkpoint(at: datetime, state: str) -> Message:
    ...
    那句"再往前的那一段不在你眼前了"只对最新这根是实话，下一根立起来时它连同状态一起
    折叠掉（:func:`_folded`）。
    """
    return _marker(
        CHECKPOINT_HEAD,
...
def _folded(message: Message) -> Message:
    """一根旧界桩只留表头（开头 + 时刻 + 收尾括号）；不是界桩就原样还回去。

    **表头是界桩上唯一有人读的部分**：取最近界桩的时刻、分代边界、按代裁剪，三处都只
    读它（:func:`_checkpoint_at`）。正文那份状态只对立起它的那一刻成立，新界桩带着一份
    新的状态立起来之后，它就是一份过时的副本 —— 留着的话一天清理十几次就叠十几份，每份
    都带着整页日记。2026-09-23 prod 那一版里叠了 5 份，她自己说过的话于是在一轮输入里
    出现了几十次，说话方式跟着一轮比一轮收窄成同一个样子。

    **不是删掉**：删了分代边界就没了，它左边那些消息的年龄无从判断。表头之后那句"再往
    前的那一段不在你眼前了"一起走 —— 放在一根已经过时的界桩上，它说的就不是实话了。

    表头永远不是空串（开头本身就有字），所以折叠之后这条仍然是一个有内容的 USER：空
    串会被 gemini adapter 编码成一个没有 parts 的 content，provider 拒掉整个请求。

    按第一个收尾括号切：两种开头和时刻里都没有这个字符，所以它就是表头的结尾，跟
    :func:`_checkpoint_at` 认的是同一个位置。
    """
    if _checkpoint_at(message) is None:
        return message
    header, tail, _state = message.content.partition(_CHECKPOINT_TAIL)
...
def trim_for_round(
    ...
    **立新界桩的那一下，之前每一根都折叠成只剩表头**（:func:`_folded`），所以上下文
    里只有最新那根带着状态。缺口那根也一样：它在小时中间立起来，那一轮前缀会多变一次，
    缓存少命中一次 —— 缺口很少，接受。

    ``state`` 里带不带"你刚做过、说过"不归这里定，调用方按
    :func:`holds_her_recent_words` 渲染好了再交进来。
    ...
    **裁在模型调用之前，不是之后。** 两个理由，都是硬的：
    ...
    if marker is None:
        ...
        # 新界桩带着此刻的状态立起来，之前每一根都只剩表头（:func:`_folded`）：上下文
        # 里的全量状态永远只有一份。只在这一下折叠，所以两次清理之间前缀照样一个字节
        # 都不动。
...
def holds_her_recent_words(
    history: list[Message], *, lost_last_round: bool
) -> bool:
    """这份历史接不接得住她最近说过的话 —— 接得住，状态里就不再给「你刚做过、说过」。

    ``history`` 是这一轮读到、还没裁的那一份；``lost_last_round`` 同
    :func:`trim_for_round`。

    **接得住 = 历史里至少有一根界桩，而且上一轮存下来了。** 有界桩说明这一天是一路连
    着写下来的：她 4 小时内说过的话、发出去的每一条（连同回执上的撤回编号）都原样在
    里面（:data:`KEPT_TOOLS`），状态里再抄一份最近 12 句，就是同一批话在一轮输入里出现
    好几遍 —— 2026-09 她的说话方式一天天收窄成同一个样子，这是测到的三处重复之一。

    接不住的时候这一段照给，两类：

      * **一根界桩都没有**：一天的第一轮、停服清库之后的第一轮，眼前什么都没有；硬顶
        把界桩全裁掉了（:func:`_under_cap` 从最老的组开始整组丢），剩下那一截说不清是
        从哪儿接上的，丢掉的可能正是她刚说过的话；
      * **上一轮没存下来**：那一轮说过的话比眼前剩下的都新，却不在眼前。

    **不按这一轮要立哪种界桩来判。** 缺口和清理点撞在一起时立的是清理那根（先判
    :func:`_due`），硬顶之后下一轮立的也是清理那根 —— 按界桩类型判，这两种情况她就看
    不到自己刚说过什么。

    代价是有意接受的：她 4 小时没开口的话，之后几轮看不到自己更早说过什么，那段时间有
    日记和挂着的事接住；缺口那根补回来的几句，会在下一个清理点连同正文一起折叠掉
    （:func:`_folded`），最早 50 分钟就看不到了 —— 那几句仍在世界账本和日记材料里，发
    出去的消息在手机页上也还有编号。
    """
    if lost_last_round:
        return False
    return any(_checkpoint_at(m) is not None for m in history)
