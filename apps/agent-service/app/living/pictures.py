...
整轮重放、这次 moment 重跑都不该让她的记录里凭空多出一张一模一样的图。派生出来的那串是
...
**地址每次现签，一次都不存。** 签名 1.5 小时就死；她看一眼、下次再看一眼、开口那一
...
class Picture(Data):
    ...
    ``what`` 是这张图是什么 —— 她画它时说的那句话、或者她找它用的那个词。她下一轮看
...
async def pictures_she_made(
    ...
    **她报一句话来指某张时走的是 ``None``**（:func:`look_at_a_picture`）—— 跟
    ...
    分页从来不是给她的形状 —— 她记不住页码，跨一轮就忘了自己翻到哪。但"接着刚看到
...
async def _keep(
    ...
    **传不上去就什么都不记**，返回 ``None``。记下一个根本不在对象存储里的句柄，她下
    一轮会拿着它反复去取一张取不到的图，而且一句报错都没有 —— 库里那一行看起来完全
    正常。
...
async def draw_a_picture(
    ...
    画你自己或者姐妹的时候，手边那份 drawing 说明里写着你们各自长什么样，还有什么样
    的要求你可以不接。用 read_a_guide 读它，读不读你自己定。

    画完你会当场看见它，它同时进了你手上那些图里，跟着一串 pic=…。**那串是你以后再
    找到它的唯一凭据**：下次你眼前不会再有这次的结果，想再看它、想把它发给谁，都
...
async def look_through_your_pictures(
    ...
    **这是你下次还能找到那些图的唯一一条路。** 你画完那一刻看见的东西，过了这一次
...
async def look_at_a_picture(
    ...
    只能看**你自己**手上的图。别人发到你手机上的图片不在这儿，那些你打开会话就直接
    看见了（look_at_phone）。
