...
世界一直有东西在到期（T3）——这两条腿一免费一低频，life 循环不参与：
  ...
  * :mod:`app.living.world`       稀疏轮次：有什么新东西该出现了吗，默认「没有」。
    这个家**外面**今天什么样（天气 / 天色 / 节气 / 上不上学 / 今天更新什么番 / 城里
    这两周有什么）是它自己手上的六只手，不再有一层代码替它去取
  * :mod:`app.living.documents`   world 手里那棵文档树（世界的设定集）：按路径 list /
    read / write / edit / delete，根目录由环境变量给。**她手里没有这几只手**——文档是
    world 的工作产物，只以"她看到的东西"的形式到达她。这棵树还有第二个写者：
    :mod:`app.nodes.world_documents` 那四个端点让人从进程外面改它，走的是同一条加锁
    路径、同一套指纹，跟 world 对等
...
所以搬进了 :mod:`app.living.persona`，不是重写也不是兼容层。
"""
