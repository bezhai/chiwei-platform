...
吗"——绝大多数群里没有人在找她，而她每次醒来要为它们逐条查库，还得在信封上一条条读
...
**一次 moment 之内是同一份。** 名单跟"此刻几点"同性质：一次 moment 里多处各算一次，她看到的会话集合
会在这次 moment 中途变化 —— 信封上没有的会话，后半程突然搜得到、发得出去。所以算过一次就锚
在这次 moment 的 ambient 状态上（:data:`app.living.scope.FEATURE_IN_SIGHT`），键是"谁 + 哪
一刻"，不同 moment 之间不会互相串。nudge 那条钟不在任何一次 moment 里，它现算现用：它算出来的名单
跟随后被唤醒的那次 moment 是**两次独立计算**，中间隔着模型调用的时间。
...
async def channels_in_sight(
    *, persona_id: str, now: datetime, conversations: list[dict]
    ...
    一次 moment 之内只算一次：算过就锚在这次 moment 上，同一个人、同一刻再问拿到的是同一份（理由
    见模块 docstring）。moment 之外（nudge 那条钟、运维查一眼）现算现用。
    """
...
async def _asked(query, /, **kwargs) -> list[dict]:
    ...
    掉出去的代价是她这次 moment 在那些会话里说不了话，下次 moment 算得出来就恢复；固定加白那几
    个群不受影响（它们不看计数）。
    ...
    try:
        ...
        logger.warning(
            ...
            "按不在名单里算（她在它们里面说不了话，下次 moment 再试）",
...
def _anchor_key(persona_id: str, now: datetime) -> str:
    ...
    拿到的是重新算的那份，不是上次 moment 留下的。一次 moment 之内 ``now`` 本来就是同一个（时间
    锚是 moment 的身份），所以这次 moment 里每一处问到的都是同一份。
    """
    return f"{persona_id}\x1f{now.isoformat()}"


def _anchored(*, persona_id: str, now: datetime) -> frozenset[str] | None:
    """这次 moment 已经定下的那份名单；不在 moment 里、或者还没算过返回 ``None``。

    不在 moment 里就没有"本次 moment 定下的"这回事（同
...
def _anchor(*, persona_id: str, now: datetime, channels: frozenset[str]) -> None:
    """把这次 moment 的名单定下来。不在 moment 里就什么都不记。"""
    try:
        ctx = get_context()
    except LookupError:
        return
