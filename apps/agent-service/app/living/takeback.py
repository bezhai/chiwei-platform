...
**她指的是编号，不是内容。** 她发出去的那一刻，发送回执里就带着它：

    发出去了［55f3469bd46c5384a9ce22cb4944b77a］

她自己的话不在眼前的那几轮（一天的第一轮、上一轮没存下来），状态里「你刚做过、说过」
那段也照同一个印法把它印在那句话后面：

    - 21:00 CST 你对 bezhai 说：「你去过那家抹茶店吗？」［55f3469bd46c5384a9ce22cb4944b77a］

方括号里那串就是 :attr:`app.living.mouth.SpokenOutbound.outbound_id`，两处都经
:func:`app.living.happening.printed_handle` 印出来（前缀常量见
:data:`app.living.records.OUTBOUND_HAPPENING_PREFIX`，全链只有那一处定义）。所以这只
手收的是那个编号，在她自己那张认领表里**按等值**找，不管她是从哪一处抄来的。

**同一个值她会在三处看到，印法有两种。** 手机那侧（:func:`app.living.phone._one_message`）
把它印成消息标签上的 ``take_back_id`` 属性，因为那儿的正文和显示名都来自别人 ——
方括号加一串 hex 是别人印得出来的，留着那个散文槽等于让别人递给她一个假编号。回执和
快照这两侧印的都是她自己这一侧的东西，别人伪造不了，所以保持方括号不动。三处是**同一
个值**；手机那侧跟快照那侧的相等关系由两侧共读的成对向量钉住
（``contracts/proactive-message-id.json`` 的 ``outbound_id_vector``）。代价是她看到形状
不同的同一串，由这只手的描述兜住。

**4 小时以后，编号多半只剩手机那一处。** 回执跟她的话一样在上下文里留 4 小时，而「你
刚做过、说过」那段只在她自己的话不在眼前时才给、只有最近 12 条。撤回一条 4 小时前的消
息很少见，而手机页上她自己那几条一直带着 ``take_back_id``。
...
同步等，就是把这一整轮卡在网络上。她下次拿起手机自己看得出来 —— 撤掉了的那条就不在
会话历史里了，没撤掉的还在。
"""
...
async def take_back_message(
    message_id: Annotated[
        ...
        Field(
            description=(
                "要撤的那条消息的编号：发这条时回执里方括号里那串，"
                "「你刚做过、说过」里那句话后面方括号里那串，"
                "或者你打开会话时那条消息的 take_back_id，照抄，别自己编"
            )
        ),
    ],
) -> str:
    ...
    同一个编号你会在三个地方见到它，**形状不全一样，但那是同一串**：

    - 你发这条时的回执里，跟在「发出去了」后面的方括号里：`发出去了［a1b2……］`
    - 「你刚做过、说过」里，跟在那句话后面的方括号里：`……」［a1b2……］`
    - 你打开会话时，你自己那条消息上的属性：`<msg from="你" … take_back_id="a1b2……">`

    哪个地方看到的都照抄进来，别自己编。当面说的话、你做的事、你回复别人的消息都
    没有这个编号，也撤不了；别人发的消息正文里印出来的那种方括号更不是编号。

    只能撤**你自己**发出去的话。

    撤回要过一会儿才落到那边：这一步只是去撤，撤没撤掉这一轮还不知道。你下次拿起
    手机就看得出来——撤掉了的那条不在会话里了，没撤掉的那条还在。

    Args:
        message_id: 那条消息的编号，从上面三处任意一处照抄。
    ...
    if line is None:
        ...
        # 编号），所以只说确定的那一件：没有这条。
        raise ValueError(
            f"你没有编号 {outbound_id} 这条消息。这串从你发它时回执里的方括号、"
            f"「你刚做过、说过」里那句话后面的方括号，或者你打开会话时自己那条消息的 "
            f"take_back_id 里照抄 —— 当面说的话没有编号，别人发的消息也没有，都撤不了。"
        )
    ...
    # 而她会据此以为那句话已经不在了。结果不在这一轮里等（那会把她卡在网络上），
    # 她下次拿起手机自己看得见。
    return (
        f"去撤「{line.said}」了。撤没撤得掉这一轮还不知道 —— "
