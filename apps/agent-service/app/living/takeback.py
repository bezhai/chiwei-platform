...
:func:`app.living.happening.own_line` 印在她眼前（前缀常量见
:data:`app.living.records.OUTBOUND_HAPPENING_PREFIX`，全链只有那一处定义）。所以这只
手收的是那个编号，在她自己那张认领表里**按等值**找。

**同一个值她会在两处看到，印法不同。** 手机那侧（:func:`app.living.phone._one_message`）
把它印成消息标签上的 ``take_back_id`` 属性，因为那儿的正文和显示名都来自别人 ——
方括号加一串 hex 是别人印得出来的，留着那个散文槽等于让别人递给她一个假编号。快照
这侧印的是她自己说过的话，别人伪造不了，所以保持方括号不动。两侧是**同一个值**，
不是同一种印法；这条相等关系由两侧共读的成对向量钉住
（``contracts/proactive-message-id.json`` 的 ``outbound_id_vector``）。代价是她在两处
看到形状不同的同一串，由这只手的描述兜住。
...
async def take_back_message(
    ...
    Args:
        message_id: 那条消息的编号，从上面两处任意一处照抄。
    ...
    if line is None:
        ...
        # 编号），所以只说确定的那一件：没有这条。
        raise ValueError(
            f"你没有编号 {outbound_id} 这条消息。这串要么从「你刚做过、说过」里那句话"
            f"后面的方括号里照抄，要么从你打开会话时自己那条消息的 take_back_id 里"
            f"照抄 —— 当面说的话没有编号，别人发的消息也没有，都撤不了。"
        )
