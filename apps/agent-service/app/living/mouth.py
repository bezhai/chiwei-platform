...
:class:`SpokenOutbound` 认领表——工具重试、整轮 ``@retry`` 重放、这次醒来重跑，全撞同一
个派生键、只出站一次。
...
# 交出去之前那一关最多占多久。判词是一次模型调用，挂住了就是把她整个 moment 卡在网络上 ——
# 这一轮她还有别的事要做。20s 沿用真人说话那侧检查的期限，不另立一个数。
#
...
class SpokenOutbound(Data):
    ...
    **这条链上有两个写者。** 一个是 :func:`send_message`（认领 → 收口），另一个是
    ...
    # 撤回是异步的，没有一个确定的失败信号（投递侧退避重投三次才进死信）。她下次
...
async def latest_outbound(
    *, lane: str, moment_id: str
    ...
    """这一轮里那条出站记录的最新一版（一轮一条时够用，测试和排查用）。"""
    sql = (
        f"SELECT * FROM ("
...
async def send_message(
    ...
    #
    #   * 名单读的是这一轮的锚（:mod:`app.living.whitelist`）。她要回的就是这一轮开
    ...
    #     话**能不能真的送到**的物理前提，读一份这一轮开头的快照等于这道保护不存在 ——
    ...
    # **图算进发送身份。** 不算的话，同一轮、同一条会话、同样的话配另一张图会撞上
    ...
    if await insert_append(claim, expected_current_ver=0) != 1:
        ...
        # 同一句话（同一轮的重入、同一泳道的两个进程），而他可能已经交出去了 ——
    ...
    try:
        ...
        #
        # 不重发不是把这句话判死：``outbound_id`` 从 ``moment_id`` 派生，下一轮就是
