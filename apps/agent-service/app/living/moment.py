...
**moment 的身份是格子，不是钟表上那一瞬**（:func:`app.living.anchor.anchor_on_grid`）。
副作用先落库、``LifeMoment`` 后落库，中间崩掉的话下一拍会重跑；锚落在格上，重试拿到
的是同一个 moment、所有派生 id 原样对上，重放退化成无害的 no-op。残余缺口（重放产出了**不
...
工具：
  ...
  * ``draw_a_picture`` / ``find_a_picture_online``  自己画一张 / 上网找现成的一张
    （:mod:`app.living.pictures`）
  * ``look_through_your_pictures`` / ``look_at_a_picture``  翻一翻她手上有哪些图 /
    拿出其中一张看（:mod:`app.living.pictures`，跟读东西那两只手同一个形状：先列出
    来、再点一张。**"看"这只手拆出来是必须的**——这个 moment 的工具结果不传给下一个 moment，句柄
    只出现在画图那一刻的话，下一个 moment 她就再也找不到自己画过的东西）
...
个——``persona_name``、``persona_core``、``guides_you_can_read``。**每个 moment 都变的东西一律
...
class LifeMoment(Data):
    ...
    **``began_at`` 和 ``seq`` 回答的是两个问题，别互相顶替。**
    ...
    算不准（会丢 trace），光看 ``Whereabouts`` 看不见"跑了但没换"的那些 moment。

    ``nudged`` 分开的是**两种 moment**：钟点上该来的那种，和被人叫来提前的那种。这一列
    ...
    class Meta:
        ...
        #   * (lane, persona_id, seq)      最后落地的那个 moment + 取下一个号的 MAX(seq)
    ...
    # 加列之前的行统一当成 0 号：新写的 moment 从 1 起，所以任何一条新 moment 都排在它们后面，
...
async def move_to(
    ...
    # 自然键跟 switch_to 同款：同一个 moment 里挪两次要落两行（最新那条才是"当前"），
    # 而重放同样的内容仍然只落一行。
    await note_whereabouts(
        lane=lane,
        persona_id=persona_id,
...
async def keep_in_mind(
    ...
    **随时都能调，跟你换不换手上的事没有关系。** 有人跟你说了句要紧的话、你临时
    想起一件事、你答应了谁什么——手上的书不用放下，记一笔就行。

    这是**整份清单，不是新增**：列进去的会一直跟着你，这次没列的就此了结、下一次你
    就想不起来了。上一份清单每次都原样摆在你眼前，照着抄，再加上新的那件。

    你没主动记下来的东西，过几次就真的没了——你眼前那份"刚发生过什么"只有最近一小
    段，滚出去就找不回来了。
    ...
    **有的事该在几点**（约好的谈话、说了要打的电话、几点出门）。那就把时刻写在这件
    ...
    # 收下了没有（下一个 moment 才在快照里看得见，那时已经隔了十分钟）。
...
async def act(
    ...
    **你写下的这句话，原样就是姐姐下一次感知到的客观动静，中间没有谁替你核一遍。**
...
# 只在这里汇成一份——拆成几份工具集就会出现"某条路进来的那个 moment 她没有手机"这种谁也想
# 不到的差别。
#
...
async def latest_regular_moment(
    *, lane: str, persona_id: str
    ...
    """这个人最近跑过的那**一格**（钟点上该来的那种 moment）；一个都没有返回 ``None``。

    间隔判断只认它。用"最近一次"判的话，每被人叫来一次就把她的固定节奏往后推一次
...
async def _next_moment_seq(*, lane: str, persona_id: str) -> int:
    ...
    **只能在 moment 的排他占用里调**（:func:`life_moment_lock_key`）：取号和落库之间不放开
    ...
    旧行的 ``seq`` 是 NULL，``MAX`` 直接忽略 NULL，所以加列后的第一个 moment 拿到 1 —— 比任何
    旧行（读出来是 0）都大。
    """
    sql = (
...
def build_moment_runner() -> AgentRunner:
    """这个 moment 的 agent。模块级函数，测试替身从这里换掉，不碰真模型。"""
...
async def run_moment(
    ...
    **她被带到那一刻，回不回是她的输出。** 这里只负责把她带到，不看她说了什么、也没有
    ...
    async with hold(life_moment_lock_key(lane, persona_id)):
        ...
        # **这个 moment 落地和她看过的手机是同一个事务。** 工具返回不等于她看见了——只有这个 moment
        # 跑完，工具结果才真的进过她的上下文。分开写的话，崩在两者之间就是"已读了但内容
        # 从没到她眼前"，那几条消息永久消失且一句报错都没有。绑在一起之后崩掉的代价只是
        # 她下一个 moment 原样再看一遍：宁可重看，不可漏看。
        async with get_session() as s:
    ...
    **并发跑，一个人炸不拖累另两个。** 三个人各有自己的占用（每人一条轴），所以
    ...
    for persona_id, outcome in zip(LIVING_PERSONAS, outcomes, strict=True):
        if isinstance(outcome, BaseException):
            logger.warning(
                "living moment lane=%s persona=%s 这个 moment 炸了：%r",
                lane,
                persona_id,
                outcome,
                exc_info=outcome,
