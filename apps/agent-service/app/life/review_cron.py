...
:func:`app.life.living_day.previous_living_day`）：marker 未落则补跑回顾。逐小时
窗口让 05:00 那班失败（模型抖动 / 部署窗口撞上）后 06:00–10:00 还有五班补，已
成功的由 marker 幂等挡住。钟纯机械、不依赖任何 agent 行为、部署杀不掉。
...
锁内 marker 对账：一个 persona 失败绝不影响下一个；与快班撞车由锁解决（撞锁方
静默让位）。
...
async def day_review_sweep_node(signal: LifeDayReviewSweep) -> None:
    """清晨对账班：对每个有 life 的 persona 检查刚结束的生活日，marker 未落则补跑回顾。

    target = ``previous_living_day(now)``（04:00 晨界后整个上午都是前一日标签，
    05:00–10:00 窗口内每班 target 相同——失败的班下一小时补、成功的由 marker
    幂等挡住）。该 lane 没有 LifeState 记录的 persona 过滤掉（没有生活日可对账，
    bot_persona 全表里可能有没有 life 的 persona）。marker 预检查是省一次锁的
    优化；权威对账在 ``run_day_review`` 锁内再做一次。run_day_review 绝不向上抛
    （fail-open），逐 persona 顺序对账互不拖累。
    """
    lane = signal.lane
    ...
    for persona_id in persona_ids:
        ...
        if snapshot is None:
            ...
            continue
        if snapshot.day_reviewed_date == target_date:
            logger.info(
                "[day_review_sweep] %s/%s %s already reviewed (fast shift), skip",
                lane,
                persona_id,
                target_date,
            )
