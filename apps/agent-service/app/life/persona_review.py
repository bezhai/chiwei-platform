...
def persona_review_instruction() -> str:
    ...
    return (
        ...
        "改完读起来仍是同一篇文章。短期的事不是身份——一阵子的不舒服、一段时间"
        "的临时安排、临时的作息、还在等结果的事，都是日子里的事件、不是她是谁，"
        "不写进身份正文；底稿里若还留着这类短期状态，把它拿掉，那不算无谓改动——"
        "拿掉的依据是它本身就是短期的事，或证据表明它已经结束，不是因为这段日子"
        "没再提到它。有期限的人生阶段（一个学期、一个暑假）仍可能是身份的一部分，"
        "按「一段经历有没有沉淀成她是谁」来判；除此之外的阶段、关系、牵挂一律"
        "不删。\n\n"
