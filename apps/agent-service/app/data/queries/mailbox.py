...
async def find_conversation_display_name(chat_id: str | None) -> str | None:
    ...
    或 display_name 为 NULL（私聊 / 未命名群）同样返回 None——调用方据 None 兜底（life
    侧群名缺失展示 ``group:<chat_id>`` 句柄）。
    """
    if not chat_id:
        return None
    try:
        chat_uuid = str(UUID(str(chat_id)))
    except ValueError:
        return None
    sql = (
    ...
    async with get_session() as s:
        result = await s.execute(text(sql), {"cid": chat_uuid})
