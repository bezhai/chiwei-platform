...
def build_life_tools(
    ...
    **Task 2 删自设闹钟**：以前这里还有一个 ``self_wake`` 容器 + ``schedule`` 工具，让她
    ...
    async def update_life_state(
        ...
        记在这里就够了。这里记的是你自己的状态。别人怎么样、外面发生了什么，不归你
        在这里记成事实——那些你只能等世界让你感知到；拿不准的就写成拿不准。想改就
        调；可以调多次（以最后一次为准），也可以一次都不调。要去睡了就把
        activity_type 标成 sleep——睡前你会回看这一天。
    ...
    async def act_tool(description: str) -> str:
        ...
        要等它真在你之外发生，你当场未必知道）。act 里把你做的事说完整——动作本身
        直接造成的结果也算（把饭端上桌、把窗推开）；但别在里面替世界宣布不由这个动作
        直接产生的事——对方现在怎么样、外面出了什么事、查出了什么结果，那些要等世界
        告诉你；你写进去了，世界也只认你做了的那件事。只在心里转了一下、顺耳听过
        刚才的动静、没在外面留下任何痕迹，就不算，不用 act。
        ...
        可能记到重试时模型给的不同措辞，但 world 推演依据的动作描述以首次落库那条为准。
