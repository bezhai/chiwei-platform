...
def _format_recent_chats(conversations: list[LifeChatConversation]) -> str:
    ...
    **段头只做输入溯源**（2026-07-25 事故的第二个落点）：每行区分自他只靠一个字「我」，
    而并列的【你刚对这次交流的回复】段有整句框架（「这是你自己刚发出去的话，不是别人对
    你说的，已经发过了、不用再回一遍」）——两段口径不一致，实测就是在这儿混淆的：她把
    自己刚发的话读回来当成别人在跟她说话，接着跟一个不存在的对话往下编。所以段头点明
    标「我」的行是她自己发出去的话。

    **段头到此为止，多一句都不加**（codex T3 驳回过一版更长的措辞，理由成立）：
      * **不许宣称全量**——真实上限是 :data:`_RECENT_CHAT_MAX_CONVERSATIONS` 个会话 ×
        每会话 :data:`_RECENT_CHAT_PER_CHAT_LIMIT` 条，还过白名单。写「这里就是全部」
        是假话，而且会让她把**截断误当成沉默**，比不提示更糟。
      * **不许下行为指令 / 给情绪框架**——「不必再说一遍」是指令，「那份安静也是真的」
        是情绪暗示。沉默本来就是可见的：没人说话，那些行就不在，不需要谁替她解读；替
        她解读反而会持续把她的注意力拽到「怎么没人说话」上（赤尾宪法：不确定性来自她
        自己的判断，不是塞给她的暗示）。
    每行的渲染格式一个字不动（忠实呈现红线仍在）。
    """
    blocks: list[str] = [
        "【最近聊过的对话】（这一阵你和谁聊过的，按会话分开。标「我」的行是你自己"
        "发出去的话。）："
    ]
