...
真 Postgres（testcontainers），只 mock 两处 LLM（``_world_deliberate`` /
``_think``）——别的全走真实持久化：mock 掉持久化等于什么都没测。``emit_delayed``
（自排）打桩成记录 delay，不连 RabbitMQ。
...
def _world_llm(monkeypatch, scripted: list[WorldDeliberation]):
    """把 world 的 LLM 推演换成脚本：依次回放每次唤醒的 deliberation。

    记录每次被喂的 wake_reason / presence_text，供断言"world 真读到了世界 +
    意图被透给 LLM 裁决"。
    """
    calls: list[dict] = []
    seq = list(scripted)
    ...
    async def fake_deliberate(*, lane, snapshot, presence_text, wake_reason):
        ...
        return seq.pop(0) if seq else WorldDeliberation()

    monkeypatch.setattr(engine_mod, "_world_deliberate", fake_deliberate)
    return calls


def _life_llm(monkeypatch, decision: lw.LifeDecision):
    """把 life 的 LLM 思考换成脚本，记录被喂的 prompt_vars（验信息差）。"""
    captured: dict = {}

    async def fake_think(*, persona_id, snapshot, unread, prompt_vars):
        captured["persona_id"] = persona_id
        captured["prompt_vars"] = prompt_vars
        captured["unread"] = list(unread)
        return decision

    monkeypatch.setattr(lw, "_think", fake_think)
    return captured


@pytest.fixture(autouse=True)
def _stub_self_wake(monkeypatch):
    """world 自排打桩成记录 delay（不连 RabbitMQ），并断言永不排长闹钟。"""
    self_wakes: list[int] = []
...
async def test_full_closed_loop_world_to_life_to_world(
    world_db, _stub_persona, monkeypatch
):
            ...
            WorldDeliberation(
                ...
                events=[
                    WorldEventDraft(
                        room_id="akao_room",
                        summary="晌午的光照进房间",
                        occurred_at="2026-06-03T12:30:00+08:00",
                    )
                ],
                next_situation="晌午，千凪在厨房，赤尾房间晒进太阳。",
            ),
            # 第二次唤醒（intent 裁决）：把 akao 挪进 kitchen + 产新 event。
            # 赤尾刚进厨房煮咖啡，world 想几分钟后再看一眼出锅没 → 给提前卡点。
            WorldDeliberation(
                presence_changes=[
                ...
                events=[
                    WorldEventDraft(
                        room_id="kitchen",
                        summary="赤尾走进了厨房",
                        occurred_at="2026-06-03T12:35:00+08:00",
                    )
                ],
    ...
    # 冷启动确实走了 LLM 推演（不是硬编死表）
    assert "冷启动" in world_calls[0]["wake_reason"] or "首次" in world_calls[0]["wake_reason"]
    ...
    monkeypatch.setattr(lw, "raise_intent", capture_raise_intent)
    ...
    blob = repr(life_captured["prompt_vars"]).lower()
    ...
    # 棒 3 交接证据：意图被透给 world LLM 裁决
    assert "煮咖啡" in world_calls[1]["wake_reason"]
    ...
    # 第二次裁决 LLM 给了 300s 提前卡点 → 自排一条 self，且夹到 ≤ 10 分钟保底心跳
    # （world 不许排长闹钟把世界睡死）。第一次冷启动没给提前需求、不自排。
...
@pytest.mark.integration
async def test_info_gap_absent_sister_gets_nothing(world_db, monkeypatch):
...
@pytest.mark.integration
async def test_big_state_interrupted_not_stuck(world_db, _stub_persona, monkeypatch):
            ...
            WorldDeliberation(
                events=[
                    WorldEventDraft(
                        room_id="classroom",
                        summary="下课铃响了",
                        occurred_at="2026-06-03T08:40:00+08:00",
                    )
                ]
            )
        ],
    ...
    async def noop_raise_intent(**kwargs):
        pass

    monkeypatch.setattr(lw, "raise_intent", noop_raise_intent)

    await lw.life_wake_node(EventArrived(lane=lane, persona_id="akao"))

    # 她读到了打断的 event（旧"在上课"被推醒重想，不卡死）
    assert "下课铃响了" in repr(life_captured["prompt_vars"])
    # 状态真的换了
    snap = await find_life_state(lane=lane, persona_id="akao")
...
async def test_batched_events_consumed_in_one_life_round(
    ...
    _world_llm(
        ...
        [
            WorldDeliberation(
                presence_changes=[PresenceChange(persona_id="akao", room_id="akao_room")],
                events=[
                    WorldEventDraft(
                        room_id="akao_room", summary="水壶在响",
                        occurred_at="2026-06-03T07:00:00+08:00",
                    ),
                    WorldEventDraft(
                        room_id="akao_room", summary="走廊有脚步声",
                        occurred_at="2026-06-03T07:00:01+08:00",
                    ),
                    WorldEventDraft(
                        room_id="akao_room", summary="窗外鸟叫",
                        occurred_at="2026-06-03T07:00:02+08:00",
                    ),
                ],
            )
        ],
    ...
    assert len(captured["unread"]) == 3
    blob = repr(captured["prompt_vars"])
    for s in ("水壶在响", "走廊有脚步声", "窗外鸟叫"):
        assert s in blob, f"攒批的 event {s!r} 没被这一轮一次性读到"
    # 三条都被标已读 → 信箱清空
    assert await list_unread_events(lane=lane, persona_id="akao") == []
...
async def test_intent_translation_routes_to_world_in_process(
    ...
    # 先种一版 WorldState，让这次唤醒不是冷启动 —— 这样 wake_reason 走 intent
    # 分支、能验意图内容经真实 emit 路由透到了 world LLM。
    from app.world.state import write_world_state

    await write_world_state(
        lane=lane,
        world_time="2026-06-03T14:00:00+08:00",
        situation="下午，三姐妹各自在家。",
    )

    world_calls = _world_llm(
        monkeypatch,
        [WorldDeliberation(events=[])],  # 裁决：克制不产 event，只验被踹醒
    )
    ...
    # world_tick 真被踹醒：读了快照、意图透给 LLM 裁决
    assert world_calls, "intent→world 回环空转：world_tick 没被 emit 路由踹醒"
    assert "看花" in world_calls[0]["wake_reason"]
