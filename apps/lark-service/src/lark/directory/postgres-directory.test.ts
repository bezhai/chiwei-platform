    ...
    function manager(inTransaction: boolean): EntityManager {
        return {
            query: async (sql: string, params: unknown[]) => {
                ...
                return sql.startsWith('SELECT') && !sql.includes('advisory')
                    ? (answers.shift() ?? [])
                    : [];
            },
...
describe('Postgres directory store', () => {
    it('acquires a transaction advisory lock before fetching and commits after applying', async () => {
        const h = rig();
        await h.store.withChatLock("oc_'quoted", async (tx) => {
            ...
            expect(h.calls[2]!.sql).toContain('idle_in_transaction_session_timeout');
            ...
            await tx.saveProfile({ unionId: 'on_1', name: "O'Brien" });
            await tx.applyMembers('oc_1', [{ unionId: 'on_1', name: "O'Brien" }], ['on_gone']);
        });
        ...
        expect(
            h.calls.find((call) => call.sql.startsWith('INSERT INTO lark_user '))!.params,
        ).toEqual(['on_1', "O'Brien", null, false]);
        expect(
            h.calls.find((call) => call.sql.startsWith('UPDATE lark_group_member'))!.params,
        ).toEqual(['oc_1', ['on_gone']]);
        expect(writes).not.toMatch(/INSERT INTO (common_user|lark_user_open_id)/);
    });
    ...
    it('rolls back all directory writes when any identity name update fails', async () => {
        ...
        await expect(
            h.store.withChatLock('oc_1', async (tx) => {
                await tx.saveProfile({ unionId: 'on_1', name: '新人' });
                await tx.applyMembers('oc_1', [{ unionId: 'on_1', name: '新人' }], []);
            }),
        ).rejects.toThrow('database unavailable');
        expect(h.lifecycle).toEqual(['begin', 'rollback']);
        expect(h.calls.some((call) => call.sql.includes('INSERT INTO lark_group_member'))).toBe(
            false,
        );
    });
    ...
    it('updates private profiles and existing identity names in one transaction', async () => {
        ...
        expect(h.calls[3]!.params).toEqual(['on_1', '真人', 'https://avatar', true]);
        expect(h.calls[4]!.sql).toContain('WHERE union_id = $1');
        expect(h.calls[5]!.sql).toContain('l.common_user_id = u.common_user_id');
    });

    it('reads only explicit real-user message evidence and preserves null membership as active', async () => {
        const h = rig();
        h.answers([
            [{ sender_union_id: 'on_human' }],
            [{ union_id: 'on_human', name: null, is_leave: null }],
        ]);
        expect(await h.store.humanUnionIds('oc_1')).toEqual(['on_human']);
        expect(h.calls[0]!.sql).toContain("raw_event->'sender'->>'sender_type' = 'user'");
        expect(h.calls[0]!.params).toEqual(['oc_1']);
        expect(await h.store.member('oc_1', 'on_human')).toEqual({
            unionId: 'on_human',
            name: null,
            hasLeft: false,
        });
        expect(h.calls[1]!.params).toEqual(['oc_1', 'on_human']);
    });
});
