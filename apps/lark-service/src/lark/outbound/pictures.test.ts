...
describe('renderLarkPictures 顺利的时候', () => {
    it('句柄现签 → 下载 → 上传，产出一行 img 节点', async () => {
        const h = harness();
        const rows = await renderLarkPictures(['pictures/cat.png'], h.deps);
    ...
    it('签的是队列里那个永久句柄本身，不是别的什么', async () => {
        ...
        // 每张图都签不出来，而且全程无报错。
        const h = harness();
        await renderLarkPictures(['2026/09/a-b-c.jpg'], h.deps);
        expect(h.signed).toEqual(['2026/09/a-b-c.jpg']);
    });

    it('多张图按给的顺序产出行', async () => {
        const h = harness({ upload: async () => `img_v3_${Math.random()}` });
        const rows = await renderLarkPictures(['a.png', 'b.png', 'c.png'], h.deps);
    ...
    it('没有图时一行都不产出，一次都不签', async () => {
        const h = harness();
        expect(await renderLarkPictures([], h.deps)).toEqual([]);
        expect(h.signed).toEqual([]);
    });
});

describe('renderLarkPictures 出问题的时候：降级成一行文字，绝不抛', () => {
...
describe('renderLarkPictures 的并发', () => {
    it('多张图并发处理，但同时在飞的下载不超过 5 个', async () => {
        ...
        const rows = await renderLarkPictures(fileNames, h.deps);
        ...
        expect(rows.every((p) => p.nodes[0]!.tag === 'img')).toBe(true);
    });
});
