 ...
 * 这个会话准不准发图。
 *
 * 会话类型取自事件里的 `chat_type`，拆分前取的是 `lark_base_chat_info.chat_mode`
 * —— 那一列本身就是从 chat_type 派生的。两者只在话题群（topic）上可能不一致，
 * 见本批的行为差异清单。
 */
function mayPost(context: LarkCommandContext): boolean {
    const chatType = context.message.chatType;
    if (chatType === 'p2p') return true;

    const groupChat = chatType === 'group' ? context.groupChat : null;
