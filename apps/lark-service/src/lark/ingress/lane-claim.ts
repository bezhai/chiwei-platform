// 泳道信封的幂等协议：**原子占位**。
//
// ⚠️ 这是**跨服务契约**。分区没做完之前（决策九的双订阅窗口）同一条信封可能被本服务
// 拿到、也可能被 channel-server 拿到，两边读写的是 redis 里同一批 key（格式见
// lane-queue.ts 的 inboundLaneDedupeKey）。所以统一 key 的格式不够，值和状态机也要
// 一样：
//
...
//
// 两边不一致的后果不对称，两个方向各错各的：
//   - 完成标记写成别的值（比如 '1'），对面读到会判成"有人正在处理"，于是一直退回队列，
//     直到 24h TTL 过期后**重新处理一遍**；
//   - 占位标记被对面当成"已处理"，那条消息会被直接 ACK 掉——**真丢**。
//
// 对应的实现在 apps/channel-server/src/infrastructure/integrations/inbound-lane-claim.ts，
// 两边各钉一条字面量断言（两个 app 是两个包，编译期对不上）。
//
// ## 为什么必须原子
//
// 「先查有没有处理过 → 处理 → 再标记」是三步，两个 Pod 能同时穿过第一步、各执行一遍
// 副作用（各回一条消息）。SET NX 是一次往返，赢家只有一个。
...
export function createLaneClaimStore(redis: () => LaneRedis): InboundLaneStore {
    return {
        claim: async (key) => {
...
export const laneClaimStore = createLaneClaimStore(() => getRedisClient());
