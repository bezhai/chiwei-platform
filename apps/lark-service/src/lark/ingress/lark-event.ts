...
export interface LarkEvent {
     ...
     * 入口收到这条事件的时刻。
     *
     * **在入口取，不在处理里取。** 飞书那两个入口是先应答、再异步处理（见
     * event-sink.ts），两者之间隔多久没有任何保证 —— 进程忙、事件排队都会拉开距离。
     * 撤回事件的报文允许不带撤回时刻，那时库里落的就是这个值（见 lark/recall-message.ts）；
     * 取晚了，落下去的是"我们什么时候腾出手处理它"，而不是"这件事什么时候发生"。
     *
     * 必填而不是可选：可选的话，新加一条入口的人漏填了也编译得过，而漏填的表现是
     * 时刻悄悄退化成处理时刻 —— 没有任何报错，只有库里一个偏晚的值。
     */
    receivedAt: Date;
    traceId?: string;
