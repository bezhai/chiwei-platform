...
const CHANNEL_SERVER_BASE = process.env.CHANNEL_SERVER_URL || 'http://channel-server:3000';

/**
 * 事件转发器
 * 将 Lark SDK 解析后的事件 POST 到目标 channel-server 实例
 */
export class EventForwarder {
    ...
    /**
     * 转发事件到 channel-server（fire-and-forget）
     */
    ...
    private async doForward(eventType: string, botName: string, params: unknown): Promise<void> {
        ...
        console.info(
            `[forwarder] ${eventType} for ${botName} → channel-server (lane: ${lane || 'default'}, trace: ${traceId})`,
        );
        ...
        if (!resp.ok) {
            ...
            console.error(
                `[forwarder] channel-server responded ${resp.status} for ${eventType}: ${body}`,
            );
        }
    }

    /**
     * 构造 channel-server URL（sidecar 根据 x-ctx-lane header 自动路由到泳道实例）
     */
    private buildUrl(): string {
        return `${CHANNEL_SERVER_BASE}/api/internal/lark-event`;
    }
