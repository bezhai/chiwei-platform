...
describe('EventForwarder', () => {
    ...
    beforeEach(() => {
        ...
        process.env.CHANNEL_SERVER_URL = 'http://channel-server:3000';
        mockResolver = {
    ...
    afterEach(() => {
        ...
        delete process.env.CHANNEL_SERVER_URL;
    });

    it('should forward to channel-server prod when no lane binding', async () => {
        ...
        expect(mockFetch).toHaveBeenCalledWith(
            'http://channel-server:3000/api/internal/lark-event',
    ...
    it('should log error when channel-server responds with non-ok status', async () => {
        mockResolver.resolve.mockImplementation(() => Promise.resolve(null));
        mockFetch.mockImplementation(() =>
            Promise.resolve({
                ok: false,
        ...
        expect(consoleSpy).toHaveBeenCalledWith(
            expect.stringContaining('channel-server responded 500'),
        );
