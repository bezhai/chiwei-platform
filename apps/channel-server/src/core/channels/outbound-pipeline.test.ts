...
import { reverseResolveForLark } from './outbound-pipeline';
import { InMemoryIdentityResolver } from './identity-resolver';

// 出站（chat-response-worker）：收到带全局 internal_*_id 的回复 →
// IdentityResolver.toChannel 反查回 channel 裸 ID → 飞书富文本 native 出站。
// worker 加 channel==='lark' 边界断言：防 T6 后误把非飞书 ID 喂飞书发送器。
// 反查不到 = fail-loud（toChannel 抛 IdentityNotFoundError），绝不静默发错。

describe('reverseResolveForLark', () => {
    it('reverse-resolves global ids back to lark channel ids', async () => {
        ...
        const out = await reverseResolveForLark({
            resolver: r,
            messageGlobalId: gm,
            chatGlobalId: gc,
            rootGlobalId: gr,
