...
// lark-only handler（指令处理深度绑死飞书 SDK/实体）。CommandRule/CommandHandler
// 入口适配 RuleMessage：
//   - CommandRule 谓词在平台无关 clearText 上判定（与改造前 RegexpMatch 等价）。
//   - CommandHandler 从 lark 私有 store 取回飞书 Message 跑不变的内部指令逻辑；
//     缺 store entry fail-loud（larkContextStore.get），绝不静默。
