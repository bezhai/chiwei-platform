...
//     key = 该消息的全局 commonMessageId（跨 channel 唯一、稳定）。
//   - 搬到 plugins/lark 的飞书谓词/handler 只拿到平台无关 RuleMessage，通过
//     闭包用 message.commonMessageId 向本 store get(key) 取回飞书 Message，
...
class LarkContextStore {
    ...
    put(commonMessageId: string, larkMessage: Message): void {
        this.byMessageId.set(commonMessageId, larkMessage);
    }

    get(commonMessageId: string): Message {
        const m = this.byMessageId.get(commonMessageId);
        if (!m) {
            throw new Error(
                `lark-only rule/handler invoked but no lark Message in store for ` +
                    `message=${commonMessageId}; fail-loud — silent skip/degrade is forbidden`,
            );
        }
        return m;
    }

    clear(commonMessageId: string): void {
        this.byMessageId.delete(commonMessageId);
    }
}
