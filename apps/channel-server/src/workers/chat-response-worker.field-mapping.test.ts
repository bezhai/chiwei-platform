...
// reverseResolveForLark。如果 publish 端（agent-service ChatResponseSegment）
// 在这三个字段里塞了飞书裸 om_*/oc_* 之类的 channel-native id，
// reverseResolveForLark 必抛 IdentityNotFoundError —— 整段回复炸（prod 已遇到
// "no message identity mapping for internal id \"om_x100b6fec...\""）。
//
// 本测试钉死：
//   (a) ChatResponsePayload 三个 id 字段都按全局 ULID 解释；
//   (b) 给个飞书裸 om_*/oc_* 会让 reverseResolveForLark fail-loud，永远不静默
...
// 后喂给 reverseResolveForLark 的关键映射；不引入 worker 全量依赖。
...
describe('chat-response 字段映射契约：payload 三个 id 字段必须是全局 ULID', () => {
    ...
    it('publish 端误把飞书裸 om_* 塞进 message_id -> reverseResolve 必抛 IdentityNotFoundError（fail-loud）', async () => {
        const r = new InMemoryIdentityResolver();
    ...
    it('publish 端误把飞书裸 om_* 塞进 root_id -> reverseResolve 必抛 IdentityNotFoundError（fail-loud）', async () => {
