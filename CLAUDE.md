...
**常见错误：查 chat-response-worker 的日志时用 `make logs APP=lark-server`，这是错的。** chat-response-worker 是独立 Deployment，必须用 `make logs APP=chat-response-worker`。同理 recall-worker、vectorize-worker 都是独立服务。
...
4. **一镜像多服务同步。** 部署 agent-service 后必须同步 release vectorize-worker；部署 lark-server 后必须同步 recall-worker 和 chat-response-worker。
